...
Однако часто они предпочитают использовать инструменты, в которые уже вложены значительные средства. Таким образом поддерживаются процессы операций служб для мониторинга обеих облачных моделей, но конечная цель — переход на предложение на основе SaaS. Так происходит не только потому, что для перехода необходимо время на планирование, ресурсы и средства. Кроме того, это объясняется тем, что ИТ-специалисты не уверены, какие продукты или службы Azure подходят для выполнения такого перехода.
...
* Рекомендуемый метод комплексного мониторинга инфраструктуры и приложений. Это, в частности, касается развертываемых решений для стандартных рабочих нагрузок, которые переносятся в Azure.

Это не практическое руководство по использованию или настройке отдельных служб и решений Azure, но при необходимости или доступности здесь будут представлены ссылки на такие источники. Из этого руководства вы узнаете, как успешно выполнить рабочую нагрузку с помощью рекомендуемых приемов и шаблонов.

Если вы не знакомы с Azure Monitor и System Center Operations Manager и хотите получить более полное представление о том, что делает их уникальными, и сравнить их, прежде чем продолжить, ознакомьтесь с [обзором наших платформ мониторинга](./platform-overview.md).
...
Ряд программных продуктов и служб для мониторинга и управления различными ресурсами, размещенными в Azure, корпоративной сети или у других поставщиков облачных служб. К ним относятся:
...
Первая версия этого руководством охватывает наши текущие платформы мониторинга — Azure Monitor и System Center Operations Manager. Здесь также описана рекомендуемая стратегия мониторинга для каждой модели облачного развертывания. Кроме того, в руководство входит первый набор рекомендаций по мониторингу, начиная со сбора данных и предупреждений.
