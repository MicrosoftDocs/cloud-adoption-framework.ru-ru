...
| [Целевые зоны Azure — варианты реализации](../ready/landing-zone/implementation-options.md) | Выберите оптимальный вариант реализации целевой зоны Azure в зависимости от плана внедрения в облако и облачной рабочей модели. |

Существующие определения схем КАФ и модули КАФ terraform обеспечивают отправную точку для реализации целевой зоны Azure. Однако несколько клиентов запросили более широкий вариант реализации, который может удовлетворить требования планов внедрения в облако корпоративного уровня. В этом выпуске добавляется **КАФ корпоративного масштаба** в параметры реализации зоны размещения Azure, чтобы заполнять ее. Ниже перечислены некоторые из статей, которые помогут вам начать работу с архитектурой КАФ Enterprise и эталонными реализациями.
...
| [Рекомендации по реализации](../ready/enterprise-scale/implementation-guidelines.md) | Проверка ими, необходимых для реализации корпоративного масштаба перед развертыванием |

Партнеры являются важным аспектом успешного внедрения в облако. В рамках руководства по облачной инфраструктуре внедрения мы добавили ссылки для демонстрации важных партнеров по ролям, &, как клиенты могут лучше привлекать партнеров. Список проверенных партнеров КАФ см. в разделе [предложения партнеров, предоставляемые](https://aka.ms/adopt/partneroffers) [специалистами по экспертам Azure](https://www.microsoft.com/en-us/solution-providers/search?cacheId=9c2fed4f-f9e2-42fb-8966-4c565f08f11e), или [Опытные специалисты](https://www.microsoft.com/azure/partners/advspec).
