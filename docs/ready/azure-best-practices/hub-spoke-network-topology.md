...
- **Снижение затрат и эффективность управления**. Централизация служб, которые могут совместно использоваться несколькими рабочими нагрузками, такими как сетевые виртуальные модули (NVA) и DNS-серверы, в одном расположении, что позволяет ИТ-отделу минимизировать избыток ресурсов и усилия по управлению.
...
Как показано на схеме, Azure поддерживает два типа центрально-лучевой конструкции. Он поддерживает связь, общие ресурсы и централизованную политику безопасности ("центр виртуальной сети" на схеме) или виртуальный тип глобальной сети ("Виртуальная глобальная сеть" на схеме) для крупномасштабных подключений между ветвями и ветвью в Azure.
...
- Управление потоком трафика TCP и UDP между зонами сети периферийных зон и Интернетом.
...
[1]: ../../_Images/azure-best-practices/network-hub-spoke-high-level.png "высокоуровневый пример концентратора и звезды"
[2]: ../../_Images/azure-best-practices/network-hub-spokes-cluster.png "кластер концентраторов и периферийных серверов"
[3]: ../../_Images/azure-best-practices/network-spoke-to-spoke.png "периферийные серверы"
