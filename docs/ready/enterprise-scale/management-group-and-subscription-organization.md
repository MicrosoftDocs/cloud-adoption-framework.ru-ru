...
- Создайте "песочницу" — группу управления верхнего уровня, чтобы позволить пользователям сразу же приступить к экспериментам с Azure. Пользователи могут поэкспериментировать с ресурсами, которые могут быть еще не разрешены в рабочих средах. Песочница обеспечивает изоляцию от сред разработки, тестирования и рабочей среды.
  - Дополнительные рекомендации по использованию группы управления "песочницы" верхнего уровня см. в [руководстве по реализации](/docs/ready/enterprise-scale/implementation-guidelines.md).
...
- Использование службы "Управление затратами Azure" и выставления счетов за агрегирование затрат. Сделайте ее доступной для владельцев приложений.
