...
- Транзитное подключение между виртуальных сетей в стандартной виртуальной глобальной сети включено из-за наличия маршрутизатора в каждом виртуальном концентраторе. Каждый виртуальный маршрутизатор виртуальной концентратора поддерживает общую пропускную способность до 50 Гбит/с.
...
- Не следует выполнять репликацию локальных концепций и архитектур ДЕМИЛИТАРИЗОВАНной зоны в Azure, так как вы можете использовать аналогичные возможности безопасности в Azure, как и в локальной среде, но реализацию и архитектуру необходимо адаптировать в облаке.
