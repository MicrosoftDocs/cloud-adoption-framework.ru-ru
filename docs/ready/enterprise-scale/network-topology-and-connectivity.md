---
title: Топология сети и подключение
description: Топология сети и подключение
...
Важно, чтобы корпоративные клиенты запланировать IP-адресацию в Azure, чтобы не перекрывать пространство IP-адресов в рамках локальных расположений и регионов Azure.
...
- Хотя адресное пространство виртуальной сети (VNet) можно добавить после создания, этот процесс требует сбоя, если виртуальная сеть уже подключена к другой виртуальной сети через пиринг виртуальных сетей, так как пиринг потребуется удалить и создать заново.
...
DNS является важнейшим разделом разработки в общей архитектуре корпоративного уровня, а некоторые клиенты могут использовать свои инвестиции в DNS, а другие могут видеть внедрение облачных решений как возможность модернизировать свою внутреннюю инфраструктуру DNS и использовать собственные возможности Azure.

**Рекомендации по проектированию:**

- Сопоставитель DNS можно использовать в сочетании с Azure Частная зона DNS для разрешения имен в нескольких локальных.

- Клиенты могут потребовать использования существующих решений DNS в локальной среде и Azure.
...
Сетевая топология, основанная на виртуальной глобальной сети Azure, является предпочтительным подходом корпоративного масштаба для крупномасштабных развертываний в масштабах регионов, где клиент должен подключить свои глобальные расположения как к Azure, так и к локальной среде. Топологию виртуальной сети WAN также следует использовать всякий раз, когда клиент намеревается использовать развертывания SD-WAN, полностью интегрированные с Azure. Виртуальная глобальная сеть используется для удовлетворения требований к крупномасштабному взаимосоединению. Поскольку это служба, управляемая корпорацией Майкрософт, она также сокращает общую сложность сети и помогает модернизировать сеть клиента.

Традиционная топология сети Azure должна использоваться для клиентов, которые планирует развертывать ресурсы только в нескольких регионах Azure, не требуется глобальная взаимосвязь, иметь небольшое количество удаленных или филиалов по регионам (менее 30) или требовать полный контроль и детализацию для ручной настройки сети Azure. Эта традиционная топология поможет этим клиентам создать защищенную сетевую основу в Azure.
...
- Виртуальные концентраторы глобальной сети заблокированы, и в них могут быть развернуты только ресурсы виртуальных сетей (VPN типа "точка — сеть", VPN-подключение типа "сеть — сеть" и ExpressRoute), брандмауэр Azure через диспетчер брандмауэра и таблицы маршрутов.
...
- Транзитивное подключение между виртуальными сетями (в пределах региона и между регионами) доступно в общедоступной предварительной версии.

- Подключение концентратора виртуальной глобальной сети к концентратору в настоящее время находится в общедоступной предварительной версии.
...
- Диспетчер брандмауэра Azure (сейчас находится в общедоступной предварительной версии) позволяет развернуть брандмауэр Azure в виртуальном концентраторе глобальной сети.
...
- Виртуальная глобальная сеть настоятельно рекомендуется для новых крупномасштабных или глобальных сетевых развертываний в Azure, когда клиентам требуется глобальное транзитное подключение между регионами Azure и локальными расположениями без необходимости вручную настраивать транзитивное маршрутизацию сети Azure.

  На следующем рисунке показан пример развертывания глобального клиента с центрами обработки данных, распределенными по Европе и США, а также большое количество филиалов в обоих регионах. Окружение глобально подключено через виртуальную сеть WAN и Global Reach ExpressRoute.

![Пример топологии сети ](./media/global-reach-topology.png)
 _рис. 3. Пример топологии сети._

- Используйте виртуальную глобальную сеть в качестве глобального ресурса подключения с виртуальным концентратором глобальной сети на регион Azure для подключения нескольких целевых зон между регионами Azure через их локальный виртуальный концентратор глобальной сети.
...
- Рекомендуется не использовать существующие локальные сети, такие как многопротокольное переключение меток (MPLS), для подключения ресурсов Azure между регионами Azure, так как сетевые технологии Azure поддерживают взаимодействие ресурсов Azure между регионами через Microsoft магистраль.
...
## <a name="traditional-azure-networking-customer-managed-topology"></a>Традиционная топология сетей Azure (управляемая клиентом)
...
- Если при настройке топологии сети в Azure предпочтение клиента имеет детальный контроль и настройка.

![Топология сети и подключение ](./media/customer-managed-topology.png)
 _рис. 4. управляемая клиентом топология сети Azure._
...
- Каналы ExpressRoute можно использовать для установления подключения между виртуальных сетей в одном географическом регионе или с помощью надстройки уровня "Премиум" для подключения между географическими регионами.
  ...
  - Пропускная способность будет ограничена единицей хранения (SKU) шлюза ExpressRoute.

  - Клиенты по-прежнему должны развертывать определяемые пользователем маршруты и управлять ими, если им требуется проверка и ведение журнала для трафика между виртуальными сетями.
...
- Использование методик BGP для повлиять на маршрутизацию ExpressRoute — это конфигурация за пределами облака Azure. Таким образом, клиент или их поставщик услуг подключения должны настроить свои локальные маршрутизаторы соответствующим образом.
...
![Топология сети и подключение](./media/hub-and-spoke-topology.png)
...
- Используйте существующую сеть клиента (MPLS и SD-WAN) для подключения расположений ветвей с корпоративным офисом. Транзит в Azure между ExpressRoute и VPN-шлюзами не поддерживается.
...
- Когда Клиент развертывает архитектуру сети hub-and-лучевой в двух регионах Azure и требуется транзитное подключение между всеми зонами в разных регионах, используйте ExpressRoute с сдвоенными каналами, чтобы обеспечить транзитное подключение для виртуальных сетей зон в регионах Azure. В этом сценарии целевые зоны могут передаваться в пределах региона через NVA в локальной виртуальной сети концентратора и в разных регионах через канал ExpressRoute, а трафик должен хаирпин на маршрутизаторах Microsoft Enterprise (MSEE). Эта схема показана на рисунке ниже.

![Топология сети и подключение ](./media/vnet-dual-circuits.png)
 _рис. 7. Проектирование подключения к целевой зоне._

- Если для клиента требуются сетевые архитектуры в более чем двух регионах Azure, а также используется глобальное транзитное подключение между зонами размещения виртуальных сетей в регионах Azure, хотя эта архитектура может быть реализована путем взаимодействия центрального центра виртуальных сетей с глобальным пирингом виртуальных сетей и использования определяемые пользователем маршруты и NVA для реализации глобальной транзитной маршрутизации, затраты на сложность и управление очень высоки. Вместо этого рекомендуется развернуть глобальную транзитную архитектуру сети с помощью виртуальной глобальной сети.

- Используйте [Azure Monitor Network Insights](https://docs.microsoft.com/azure/azure-monitor/insights/network-insights-overview) (в настоящее время в предварительной версии) для мониторинга сквозного состояния сетей клиентов в Azure.
...
- ExpressRoute Global Reach (если доступно) позволяет клиентам подключать локальные расположения, используя каналы ExpressRoute для транзитного трафика через магистральную сеть Майкрософт.
...
- Не используйте явно каналы ExpressRoute из одного расположения пиринга, так как это создает единую точку отказа и делает клиент уязвимым для сбоев в связи с одноранговым расположением.
...
- Корпоративные клиенты часто сталкиваются с проблемами общедоступных конечных точек для служб PaaS, которые должны быть соответствующим образом устранены.
...
- Используйте внедрение виртуальной сети для поддерживаемых служб Azure, чтобы сделать их доступными в клиентской виртуальной сети.
...
- Собственные службы безопасности сети Azure, такие как брандмауэр Azure, брандмауэр веб-приложения Azure (WAF) в шлюзе приложений Azure, а также передняя дверца Azure — это полностью управляемые службы, поэтому клиенты не повышают затраты на операционные ресурсы и управление, связанные с развертываниями инфраструктуры, которые могут быть сложными в масштабе.

- Архитектура корпоративного уровня полностью совместима со сторонними NVA, если клиент предпочитает использовать NVA или в ситуациях, когда собственные службы не соответствуют конкретным требованиям клиентов.
...
- Используйте брандмауэр Azure, чтобы управлять:
  ...
  - Фильтрация трафика "Восток-Западная" (если это требуется для клиента)

- Используйте диспетчер брандмауэра с виртуальной глобальной сетью для развертывания и управления брандмауэрами Azure через виртуальные глобальные сети или концентраторы виртуальных сетей. Сейчас диспетчер брандмауэра находится на этапе предварительной версии для виртуальной глобальной сети и регулярного виртуальных сетей.
...
- Настройте поддерживаемые сторонние поставщики безопасности SaaS в диспетчере брандмауэра, если клиент хочет использовать такие решения для защиты исходящих подключений.
...
- Используйте [планы защиты Azure от атак DDoS Standard Protection](https://docs.microsoft.com/azure/virtual-network/ddos-protection-overview) для защиты всех общедоступных конечных точек, размещенных в виртуальных сетей клиента.

- Не следует выполнять репликацию локальных концепций и архитектур ДЕМИЛИТАРИЗОВАНной зоны в Azure, так как клиенты могут получить аналогичные возможности безопасности в Azure, как в локальной среде, но реализация и архитектура потребуется адаптировать к облаку.
...
- Группа безопасности приложений должна быть использована на уровне подсети группы безопасности сети для защиты многоуровневых виртуальных машин в их целевой зоне.
...
- Используйте группы безопасности сети для выборочного список разрешений подключения между зонами.

- Для топологий виртуальных глобальных сетей направьте трафик между зонами через брандмауэр Azure, если клиенту требуются возможности фильтрации и ведения журнала для трафика, передаваемого между зонами.
...
**Рекомендации по проектированию:** 
 ![ Потоки шифрования ](./media/enc-flows.png)
...
- При использовании ExpressRoute Direct настройте [Управление доступом к носителю (максек)](https://docs.microsoft.com/azure/expressroute/expressroute-howto-MACsec) , чтобы зашифровать трафик на уровне 2 между маршрутизаторами клиента и MSEE, как показано на `Flow B` схеме выше.
...
- Используйте сторонние NVA в Azure для шифрования трафика через частный пиринг ExpressRoute на случай, если собственные решения, предлагаемые в Azure (как показано в последовательностях B и C выше), не соответствуют требованиям клиентов.

## <a name="planning-for-traffic-inspection"></a>Планирование проверки трафика

Во многих отраслях клиентам требуется, чтобы трафик в Azure, в частности входящий и исходящий Интернет-трафик, был зеркально отражен в сборщик сетевых пакетов для глубокой проверки и анализа. В этом разделе рассматриваются ключевые моменты и Рекомендуемые подходы к зеркальному отображению или касанию трафика в виртуальной сети Azure.
