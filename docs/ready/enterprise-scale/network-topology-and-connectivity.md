...
- Используйте [Azure Monitor Network Insights](/azure/azure-monitor/insights/network-insights-overview) (в настоящее время в предварительной версии) для мониторинга сквозного состояния сетей в Azure.
...
- Обычно доступ к службам Azure PaaS осуществляется через общедоступные конечные точки. Однако Платформа Azure предоставляет возможности для защиты таких конечных точек или даже сделать их полностью закрытыми:
  ...
  - [Частная ссылка](/azure/private-link/private-endpoint-overview#private-link-resource) предоставляет выделенный доступ, используя частные IP-адреса для экземпляров Azure PaaS или пользовательских служб за Azure Load Balancer стандарт.
