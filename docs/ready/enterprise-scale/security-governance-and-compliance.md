...
Шифрование — это важный шаг для обеспечения конфиденциальности данных, обеспечения соответствия требованиям и местонахождение данных в Microsoft Azure. Это также одна из наиболее важных проблем безопасности многих корпоративных клиентов. В этом разделе мы рассмотрим рекомендации по проектированию и рекомендации по их отношению к шифрованию и управлению ключами.
...
- Смена ключа и истечение срока действия секрета.
  ...
  Закупок и подписывание сертификатов с помощью Key Vault: ([о сертификатах](https://docs.microsoft.com/azure/key-vault/certificates/about-certificates)) оповещения, уведомления и автоматическое продление сертификатов.
...
- Требования к использованию управляемых клиентом ключей для собственных механизмов шифрования, таких как шифрование службы хранилища (SSE): [ключи, управляемые клиентом](https://docs.microsoft.com/azure/storage/common/storage-encryption-keys-portal))
...
- Используйте модель федеративного Key Vault, чтобы избежать ограничений шкалы транзакций.

- Подготавливайте Key Vault с включенными политиками обратимого удаления и очистки, чтобы обеспечить защиту удаленных объектов.
...
Важно, чтобы корпоративные клиенты имели возможность видеть, что происходит в своем техническом облаке. Мониторинг безопасности и ведение журнала аудита служб платформы Azure — ключевой компонент масштабируемой платформы.
...
Важно поддерживать работоспособность системы безопасности, так как корпоративные клиенты принимают Azure. Помимо видимости, необходимо иметь возможность управлять исходными параметрами и изменениями по мере развития служб Azure. Поэтому планирование безопасности платформы является ключом.
...
- Совместное изучение каждой требуемой службы должно проводиться в контексте базовых требований клиентов. Если клиент хочет использовать собственные ключи, он может не поддерживаться во всех признанных службах. Необходимо реализовать соответствующие меры, чтобы несогласованность не ухудшит желаемые результаты. Выберите соответствующие пары регионов и регионы аварийного восстановления, которые снижают задержку.

- Разработка плана безопасности список разрешений для оценки конфигурации безопасности служб, мониторинга, оповещений и интеграции их с существующими системами.

- Определите план реагирования на инциденты для служб Azure перед список разрешений.

- Используйте возможности отчетов Azure AD для создания отчетов аудита контроля доступа.

- Выровняйте требования к безопасности клиентов с помощью карт для платформы Azure, чтобы оставаться в курсе новых элементов управления безопасностью.
...
|                              |                                                                       | Взаимодействует ли служба поддержки Майкрософт с аудитом службы?                                                                               |
