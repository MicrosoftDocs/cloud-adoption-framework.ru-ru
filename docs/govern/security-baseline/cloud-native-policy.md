...
- **Методика отработки отказа.** Облачные операции с низкой допустимостью для сбоя должны иметь возможность отработки отказа и (или) восстановления из кибербезопасности или инцидента платформы. Эти процедуры не должны просто задокументированы, но рекомендуется ежеквартально.
