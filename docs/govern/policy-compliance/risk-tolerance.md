...
Определение рисков — это достаточно прямой процесс. Риски, связанные с ИТ, обычно являются стандартными для разных отраслей. Допуск для этих рисков зависит от конкретной организации. На этом этапе диалога между компанией и ИТ-командой часто возникает затруднение. Каждая сторона говорит на своем языке. Приведенные далее сравнения и вопросы помогут сторонам построить продуктивный диалог, чтобы лучше понять и рассчитать допустимость рисков.

## <a name="simple-use-case-for-comparison"></a>Простые примеры для сравнения

Чтобы составить представление о допустимости рисков, рассмотрим данные клиентов. Если компания в любой отрасли отправляет данные о клиентах на незащищенный сервер, технический риск того, что эти данные будут скомпрометированы или украдены, примерно одинаково. Допуск для этого риска будет отличаться в зависимости от природы и потенциальных значений данных.

- Медицинские и финансовые компании в США подчиняются жестким сторонним требованиям соответствия. Предполагается, что личные данные или данные, связанные с здравоохранение, чрезвычайно конфиденциальны. Существуют серьезные последствия для таких типов компаний, если они участвуют в сценарии с рисками выше. Допустимость риска для нее будет очень низкой. Все данные клиентов, опубликованные в сети или за ее пределами, должны управляться сторонними политиками соответствия.
...
- Уровень предприятия среднего размера, который предоставляет дорожки службы для тысяч клиентов, будет находиться между этими двумя крайними значениями. Данные клиентов могут быть более надежными, включая такие сведения, как адреса и номера телефонов. Оба эти варианта считаются личными данными и должны быть защищены, но никаких конкретных требований к управлению не требует обязательной защиты данных. Со стороны ИТ-отдела ответ простой — данные нужно защищать. Со стороны компании могут быть нюансы. Чтобы определить уровень допустимости этого риска, компании понадобится больше информации.
...
Это только основные вопросы. На практике их будет гораздо больше. После изучения работоспособного диалогового окна рекомендуется записать соответствующие риски и, если возможно, количественно.
...
Эти вопросы посвящены упрощению технических решений, необходимых для управления и устранения рисков, но они обмениваются этими решениями способами, которые бизнес может быстро интегрировать в процесс принятия решений.

**Вероятность потери:** Вопросы, чтобы определить, насколько вероятно, что риск станет реальностью. Вероятность сложнее всего измерить. Вместо этого мы рекомендуем, чтобы Группа управления облачными клиентами создали категории для связи вероятности на основе вспомогательных данных. Приведенные ниже вопросы помогут разработать категории, которые будут понятны команде.
...
- **Будущий риск:** Текущая вероятность невелика. Продолжение внедрения потребует нового анализа.
...
- **Уменьшение риска:** Риск имеет средний и высокий уровень. Действия в ИТ-подразделении или бизнес снижают вероятность воздействия.
