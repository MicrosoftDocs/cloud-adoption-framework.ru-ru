...
- **Модель разрешений с наименьшими привилегиями**, при которой пользователи не имеют разрешений по умолчанию. Модель должна поддерживать следующее.
  ...
  - Предприятие не хочет управлять ролями независимо в каждой из трех сред, и поэтому требует использования только [встроенных ролей](https://docs.microsoft.com/azure/role-based-access-control/built-in-roles) , доступных в управлении доступом на основе ролей Azure (RBAC). Если предприятие использует настраиваемые роли RBAC, для синхронизации пользовательских ролей в трех средах потребуется дополнительный процесс.
...
![Учетная запись Azure с диспетчером учетных записей Azure](../../_images/govern/design/governance-3-0.png)
...
![Учетная запись Azure с диспетчером учетных записей Azure](../../_images/govern/design/governance-3-0a.png)
...
![Администратор службы подписки с ролью](../../_images/govern/design/governance-2-1.png)
...
![Владелец учетной записи Azure добавляет учетную запись владельца подписки в подписку с ролью владельца. Рис. *7. Владелец учетной записи Azure добавляет учетную запись **владельца подписки** в подписку с ролью **владельца** .* ](../../_images/govern/design/governance-3-0c.png)
...
5. Администратор учетных записей Azure [добавляет к каждой подписке ее владельца](https://docs.microsoft.com/azure/billing/billing-add-change-azure-subscription-administrator#assign-a-user-as-an-administrator-of-a-subscription).
