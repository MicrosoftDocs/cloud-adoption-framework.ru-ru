...
12. Команда кибербезопасности настаивает на блокировке поставщика от создания ключей шифрования на платформе поставщика облачных служб. Хотя это утверждение является необоснованным, сейчас оно принято командой как рабочее.
...
9. По возможности средства управления конфигурациями узлов должны применять требования политики к конфигурации любой гостевой операционной системы. Управление конфигурацией узла должно учитывать существующие инвестиции в объекты групповой политики (GPO) для настройки ресурсов.
...
16. Тенденции и эксплойты, которые могут повлиять на облачные развертывания, должны регулярно проверяться группой безопасности, чтобы предоставлять обновления средств безопасности, используемых в облаке.
...
21. Процессы управления облаком должны включать ежеквартальные обзоры с группами базовых показателей удостоверений для идентификации вредоносных субъектов или шаблонов использования, которые должны быть предотвращены конфигурацией облачных ресурсов.
...
3. Создание политик Azure. Создайте политику с именем `hub NSG enforcement` , чтобы принудительно настроить группу безопасности сети, назначенную любой виртуальной сети, созданной в этой подписке. Примените встроенные политики для гостевой настройки следующим образом:
...
4. Создайте проект корпоративного ИТ.
    1. Создайте схему Azure с именем `corporate-it-subscription`.
    2. Добавьте центрально-и резервные шаблоны и `hub NSG enforcement` политику.
