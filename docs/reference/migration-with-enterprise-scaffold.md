...
> [!NOTE]
> Формирование шаблонов Azure Enterprise интегрировано в платформу внедрения Microsoft Cloud. Содержимое этой статьи теперь представлено в [методологии готовности](../ready/index.md) платформы. Эта статья будет признана устаревшей в начале 2020. Чтобы приступить к использованию нового процесса, ознакомьтесь со статьей [Обзор методологии готовности](../ready/index.md), [целевые зоны Azure](../ready/landing-zone/index.md)и [Целевая зона](../ready/considerations/index.md).
...
Основой шаблона является иерархия и отношение Соглашение Enterprise (EA) к подпискам и группам ресурсов. Регистрация определяет форму и использование служб Azure в компании с контрактной точки зрения. В Соглашение Enterprise можно дополнительно разделить среду на подразделения, учетные записи, подписки и группы ресурсов в соответствии со структурой Организации.
...
Ниже приведены три стандартных шаблона для регистрации EA.
...
Чтобы защитить доступ к ресурсам, сначала нужно настроить поставщик удостоверений, а затем настроить роли и доступ. Служба Azure Active Directory (Azure AD), подключенная к вашей локальной Active Directory, лежит в основе идентификации Azure. Однако Azure AD **не** совпадает с локальной Active Directory, и важно понимать, что такое клиент Azure AD и как он связан с регистрацией. Ознакомьтесь с [управлением доступом к ресурсам в Azure](../govern/resource-consistency/resource-access-management.md) , чтобы получить полное представление об Azure AD и локальной Active Directory. Чтобы подключиться к локальному каталогу и синхронизировать его с Azure AD, установите и настройте [средство Azure AD Connect](/azure/active-directory/connect/active-directory-aadconnect) в локальной среде.

![Схема архитектуры Active Directory](../_images/reference/ad-architecture.png)

При первоначальном выпуске Azure элементы управления доступом к подписке были простыми: администратор или соадминистратор. В классической модели доступ к подписке подразумевал доступ ко всем ресурсам на портале. Отсутствие детального управления, которое привело к росту подписок, обеспечивает уровень разумного контроля доступа для регистрации. Больше такое увеличение числа подписок не требуется. С помощью управления доступом на основе ролей Azure (Azure RBAC) можно назначать пользователей стандартным ролям, которые предоставляют общий доступ, например "владелец", "участник" или "читатель", или даже создавать собственные роли.

При реализации управления доступом на основе ролей Azure настоятельно рекомендуется следующее.
...
- Используйте группы управления для назначения [ролей](/azure/azure-resource-manager/management-groups-overview#management-group-access) в нескольких подписках и снижения затрат на управление ими на уровне подписки.
...
![Схема, на которой показан глубокий мониторинг приложений, глубокий Мониторинг инфраструктуры, мониторинг ядра и общие возможности.](../_images/reference/monitoring.png)
...
- **Журнал действий:** В журнале действий описываются все операции с ресурсами в ваших подписках. Он предоставляет журнал аудита для определения того, _что_, _кто_ и _когда_ выполняет любая операция создания, обновления и удаления ресурсов. События журнала действий хранятся на платформе и доступны для запроса в течение 90 дней. Журналы действий можно получать в Log Analytics для более длительных периодов хранения и более глубокого запроса и анализа по нескольким ресурсам.
...
- **Стоимость ресурса подписки:** На портале представление " [Управление затратами Azure](/azure/cost-management-billing/cost-management-billing-overview) " предоставляет краткий обзор затрат и сведений о ежедневных расходах ресурсов или групп ресурсов.
...
- **API-интерфейсы использования Azure:** [Интерфейсы API потребления](/rest/api/consumption) предоставляют программный доступ к данным о затратах и использовании, а также сведения о бюджетах, зарезервированных экземплярах и платах Marketplace. Эти API-интерфейсы доступны только для регистраций EA и некоторых подписок Web Direct, однако они дают возможность интегрировать данные о затратах в собственные средства и хранилища данных. Вы также можете [получить доступ к этим API через Azure CLI](/cli/azure/consumption).
...
- **Используйте Azure Reserved VM Instances.** Еще одним ключевым принципом клиента для управления расходами в облаке является использование правильного средства для работы. Если у вас есть виртуальная машина IaaS, которая должна оставаться непрерывной, использование зарезервированного экземпляра позволит сэкономить значительные деньги. Поиск правильного баланса между автоматизацией завершения работы виртуальных машин и использованием зарезервированных экземпляров требует выполнения и анализа.
...
- **Azure Cloud Shell** — это интерактивная [оболочка](/azure/cloud-shell/overview) на основе браузера для управления ресурсами в Azure. Она обеспечивает полную среду для PowerShell или Bash, которая запускается по мере необходимости (и поддерживается для вас), чтобы получить согласованную среду для запуска сценариев. Azure Cloud Shell предоставляет доступ к дополнительным инструментам, уже установленным — для автоматизации среды, в том числе [Azure CLI](/cli/azure/get-started-with-azure-cli), [terraform](/azure/virtual-machines/linux/terraform-install-configure) и растущего списка дополнительных [средств](https://azure.microsoft.com/updates/cloud-shell-new-cli-tools-and-font-size-selection) для управления контейнерами, базами данных (sqlcmd) и т. д.
