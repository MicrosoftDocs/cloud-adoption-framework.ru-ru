...
- **Нарушение данных:** Основной риск при любом переходе связан с защитой данных. Утечка данных может привести к значительному повреждению компании, появлению ущерба для клиентов, снижению бизнеса или даже юридическим обязательствам. Любые изменения способа хранения, обработки или использования данных порождают риски. Облачные преобразования создают высокую степень изменения в отношении управления данными, поэтому риск не следует делать нелегко. [Базовый уровень безопасности](../security-baseline/index.md), [классификация данных](./what-is-data-classification.md)и [добавочное рационализация](../../digital-estate/rationalize.md#incremental-rationalization) могут помочь в управлении этим риском.
...
- Компания, в которой данные клиентов ограничены именем пользователя, временем воспроизведения и высокими оценками, не так высока, насколько это может негативно повлиять, если они приведут к возникновению опасного поведения выше. Хотя любые незащищенные данные подвергаются риску, последствия этого риска невелики. Таким образом, для риска в данном случае существует высокий допуск.
...
- Может ли этот риск привести к недовольству клиентов? Могут ли эти возможности повлиять на реализацию продаж или дохода?
...
**Вероятность потери:** Эти вопросы помогут понять, какова вероятность того, что риск станет реальным событием. Вероятность сложнее всего измерить. Вместо этого мы рекомендуем, чтобы Группа управления облачными клиентами создали категории для связи вероятности на основе вспомогательных данных. Приведенные ниже вопросы помогут разработать категории, которые будут понятны команде.
