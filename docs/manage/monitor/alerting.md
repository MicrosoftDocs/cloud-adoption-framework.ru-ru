...
- **База данных метрик Azure Monitor**: база данных временных рядов, которая в основном используется для Azure Monitor метрик платформы, но также содержит Application Insights данные метрик. Сведения, вводимые в эту базу данных, имеют наибольшее время оповещения.

- **Хранилище журналов Application Insights**. база данных, в которой хранятся данные телеметрии большинства Application Insights в форме журнала.
...
- **Хранилище журнала действий**. используется для всех событий работоспособности журнала действий и службы. Возможен выделенный оповещение. Содержит события уровня подписки, происходящие с объектами в подписке, как видно за пределами этих объектов. Например, если политика задана или к ресурсу осуществляется доступ или удаление.

- **Хранилище Azure**: хранилище общего назначения, поддерживаемое система диагностики Azure и другими средствами мониторинга. Это экономичный вариант долгосрочного хранения данных телеметрии мониторинга. Предупреждения не поддерживаются данными, хранящимися в этой службе.

- **Концентраторы событий**. обычно используются для потоковой передачи данных в локальные или другие партнерские средства мониторинга или ITSM.
...
- [Оповещение журнала действий](https://docs.microsoft.com/azure/azure-monitor/platform/alerts-activity-log). оповещения об элементах в хранилище журналов действий за исключением данных о работоспособности службы.

- [Оповещение о работоспособности службы](https://docs.microsoft.com/azure/azure-monitor/platform/alerts-activity-log-service-notifications?toc=%2fazure%2fservice-health%2ftoc.json)— особый тип оповещения для проблем работоспособности служб, поступающих из хранилища журналов действий.
...
Обратите внимание на [ограничения](https://docs.microsoft.com/azure/azure-subscription-service-limits#azure-monitor-limits) на количество оповещений, которые можно создать. Некоторые ограничения (но не все из них) можно увеличить, вызвав поддержку.
