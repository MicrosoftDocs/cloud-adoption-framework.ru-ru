...
Чтобы сбалансировать портфель, лучше отразить решения заката и снятия с учета, специалисты по облачным стратегиям и группе внедрения в облако должны задать следующие вопросы о каждой рабочей нагрузке на этапах оценки и миграции:
...
Если ответ на любой из этих вопросов равен «нет», то Рабочая нагрузка может быть кандидатом на выход. Если возможность выхода из эксплуатации подтверждена владельцем приложения, то не имеет смысла переносить рабочую нагрузку. Тут возникает несколько квалификационных вопросов:
