...
Влияние брандмауэра | Да/нет | Взаимодействует ли приложение с Интернетом или сетью за пределами трафика?  Интегрируется ли оно с брандмауэром?
Проблемы с безопасностью | Да/нет | Известны какие-либо проблемы безопасности, связанные с приложением?  Использует ли приложение незашифрованные данные или устаревшие платформы?
Стратегия миграции | Повторное размещение, рефакторинг, перепроектирование или перестроение | Какого типа миграция необходима для приложения? Как приложение будет развернуто в Azure? [Дополнительные сведения](./contoso-migration-overview.md#migration-patterns)
...
2. Оценка готовности. Позволяет определить, подходят ли локальные компьютеры для работы в Azure. Оценка включает в себя следующее.
...
##### <a name="use-site-recovery-at-scale"></a>Использование Site Recovery в масштабе

Компания Contoso планирует выполнить несколько миграций с прогнозированием и сдвигом. Чтобы обеспечить успешную миграцию, Site Recovery будет реплицировать пакеты по 100 виртуальных машин за раз. Чтобы выяснить, как это будет происходить, компании Contoso необходимо запланировать емкость для предложенной миграции Site Recovery.
...
**Сеть** | Компания Contoso изучила текущую инфраструктуру VPN-подключений типа "сеть — сеть" и решила реализовать Azure ExpressRoute. Эта реализация важна, так как она уменьшит задержку и повысит пропускную способность для основного региона Azure "Восточная часть США 2", используемого компанией.<br/><br/> **Мониторинг:** Компания Contoso должна тщательно отслеживать данные, передаваемые с сервера обработки. Если объем данных превысит возможности пропускной способности сети, компания рассмотрит [регулирование пропускной способности сервера обработки](https://docs.microsoft.com/azure/site-recovery/site-recovery-plan-capacity-vmware#control-network-bandwidth).
...
- Это обеспечивает возможности децентрализованной автоматизации для снижения затрат. Функции:
...
- Помимо использования встроенных функций, компании Contoso необходимо обеспечить восстановление после сбоев, избежать дорогостоящего прерывания работы, выполнить требования соответствия и защитить данные от программ-шантажистов и ошибок пользователей. Для этого выполните следующее действие.
