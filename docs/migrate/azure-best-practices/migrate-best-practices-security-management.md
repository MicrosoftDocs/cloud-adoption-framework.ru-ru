...
В дополнение к оценкам и рекомендациям центр безопасности предоставляет другие функции безопасности, которые можно включить для конкретных ресурсов.

- **JIT-доступ.** Уменьшите зону сетевой атаки с помощью JIT, управляемого доступа к портам управления на виртуальных машинах Azure.
...
- Дополнительные сведения о [центре безопасности Azure](/azure/security-center/security-center-introduction).
...
### <a name="encryption-for-infrastructure-as-a-service"></a>Шифрование для инфраструктуры как услуги

- **Виртуальные машины:** Для виртуальных машин можно использовать шифрование дисков Azure для шифрования дисков виртуальной машины IaaS (инфраструктура как услуга) Windows и Linux.
  ...
  - При использовании шифрования дисков Azure данные виртуальных машин IaaS защищаются при хранении (на диске) и во время загрузки виртуальной машины.
    - Центр безопасности предупреждает о незашифрованных виртуальных машинах.
...
### <a name="encryption-for-platform-as-a-service"></a>Шифрование для платформы как услуги

В отличие от IaaS, в котором вы управляете собственными виртуальными машинами и инфраструктурой, платформа модели "платформа как услуга" (PaaS) управляется поставщиком. Вы можете сосредоточиться на основной логике и возможностях приложения. Учитывая большое количество типов служб PaaS, в целях безопасности каждая служба будет оцениваться по отдельности. В качестве примера давайте посмотрим, как можно включить шифрование для базы данных SQL Azure.
...
- **Always encrypted:** Используйте мастер Always Encrypted в SQL Server Management Studio для защиты неактивных данных.
  ...
  - Скорее всего, для использования этой функции потребуется внести изменения в приложение.
...
- Ознакомьтесь с [обзором Always encrypted](/azure/azure-sql/database/always-encrypted-azure-key-vault-configure).
- Ознакомьтесь со сведениями о [TDE для базы данных SQL и Azure синапсе](/azure/azure-sql/database/transparent-data-encryption-tde-overview).
- Дополнительные сведения о [базе данных SQL Azure TDE с ключом, управляемым клиентом](/azure/azure-sql/database/transparent-data-encryption-byok-overview).
...
- В центре безопасности можно найти виртуальные машины, на которых не запущена программа Endpoint Protection, и установить антивредоносную программу Майкрософт по мере необходимости.
   ...
   *Рис. 1. Защита от вредоносных программ для виртуальных машин.*
...
- **Брандмауэр веб-приложения:** Это функция шлюза приложений Azure, которая обеспечивает централизованную защиту для веб-приложений.
  ...
  - Брандмауэр веб-приложения можно отслеживать с помощью Azure Monitor. Брандмауэр веб-приложения интегрирован в центр безопасности.

  ![Схема Azure Key Vault и безопасных веб-приложений. ](./media/migrate-best-practices-security-management/web-apps.png)
   *Рис. 2. Azure Key Vault.*

**Подробнее:**

- Ознакомьтесь с [обзором Azure Key Vault](/azure/key-vault/general/overview).
- Сведения о [брандмауэре веб-приложения](/azure/web-application-firewall/ag/ag-overview).
- Ознакомьтесь с [введением в среды службы приложений](/azure/app-service/environment/intro).
- Узнайте, как [настроить веб-приложение для чтения секретов из Key Vault](/azure/key-vault/general/tutorial-net-create-vault-azure-web-app).
...
Перенос рабочих нагрузок и их запуск в Azure затрагивает множество сотрудников с доступом к рабочим нагрузкам. Вашей группе безопасности следует регулярно проверять доступ к вашему арендатору и группам ресурсов Azure. Azure предоставляет предложения по управлению удостоверениями и безопасностью управления доступом, включая управление доступом на основе ролей Azure (Azure RBAC) для авторизации разрешений на доступ к ресурсам Azure.

- Azure RBAC назначает разрешения на доступ для субъектов безопасности. Субъекты безопасности представляют пользователей, группы (набор пользователей), субъекты-службы (удостоверения, используемые приложениями и службами) и управляемые удостоверения (удостоверение Azure Active Directory автоматически управляется Azure).
- С помощью Azure RBAC можно назначать роли субъектам безопасности (таким как владелец, участник и читатель) и определениям ролей (коллекция разрешений), которые определяют операции, которые могут выполнять роли.
- В Azure RBAC также можно задать области, которые задают границу для роли. Область может быть задана на нескольких уровнях, включая группу управления, подписку, группу ресурсов или ресурс.
...
- Дополнительные сведения об [Azure RBAC](/azure/role-based-access-control/overview).
- Узнайте, как управлять доступом с помощью [Azure RBAC и портал Azure](/azure/role-based-access-control/role-assignments-portal).
...
Azure AD предоставляет журналы действий, которые отображаются в Azure Monitor. В журналы записываются сведения об операциях, выполняемых в рамках аренды Azure, и пользователе, который их проводил, а также время выполнения таких операций.
...
- Сделайте это обычным, чтобы проверить журналы или интегрировать средства управления сведениями о безопасности и событиями (SIEM) для автоматической проверки на ненормальные работы. Если вы не используете лицензию Premium, необходимо выполнить массовый анализ самостоятельно или с помощью системы SIEM. Анализ включает в себя поиск рискованных действий входа и событий, а также других шаблонов атак пользователей.
   ...
   *Рис. 4. пользователи и группы Azure AD.*
...
- **Реализуйте административные единицы Azure AD (AU).** Делегирование административных обязанностей персоналу службы поддержки может быть непростой задачей, если используются только базовые средства управления доступом в Azure. Предоставить персоналу службы поддержки доступ к администрированию групп в Azure AD — не самый идеальный метод обеспечения безопасности организации. С помощью AU вы можете разделить ресурсы Azure на контейнеры аналогичным образом в локальных подразделениях (OU). Чтобы использовать Австралия, администратор AU должен иметь лицензию Azure AD уровня "Премиум". Дополнительные сведения см. [в статье Управление административными единицами в Azure AD](/azure/active-directory/users-groups-roles/directory-administrative-units).
...
- Ознакомьтесь с планом поддержки Azure и его реализацией, получите рекомендации по обеспечению актуальности виртуальных машин и разместите процессы для управления изменениями.
...
При синхронизации локальной Active Directory с Azure AD с помощью Azure AD Connect рекомендуется сопоставлять имена групп безопасности в локальной среде с именами групп ресурсов в Azure.
  ...
  *Рис. 5. Именование группы ресурсов.*
...
Случайное удаление группы ресурсов крайне нежелательно. Рекомендуется реализовать блокировку удаления, чтобы это не произошло.
  ...
  *Рис. 6. удаление блокировок.*

**Подробнее:**

- Сведения о [блокировке ресурсов для предотвращения непредвиденных изменений](/azure/azure-resource-manager/management/lock-resources).
...
- Сведения об [эталонных архитектурах Azure](/azure/architecture/browse/).
- Ознакомьтесь с [примерами сценариев Azure](/azure/architecture/browse/).
...
Если в вашей организации несколько подписок, для них необходимо управлять доступом, политиками и соответствием требованиям. Группы управления Azure обеспечивают высокий уровень охвата подписок. Ниже приведен ряд советов.
...
Планирование BCDR — это важное упражнение, которое необходимо выполнить в рамках процесса планирования миграции Azure. В юридических условиях ваши контракты могут включать предложение *Force форс мажорного* , которое оправдано обязательства из-за более высокой силы, например ураганами или землетрясения. Но необходимо убедиться, что при аварийном обеспечении все нужные службы продолжают выполняться и восстанавливаться. Возможность сделать это может оказаться критически важной для будущего вашей компании.
...
- Узнайте, как [создать моментальный снимок BLOB-объекта](/azure/storage/blobs/snapshots-overview).
...
- Прочитайте об [обратимом удалении больших двоичных объектов](/azure/storage/blobs/soft-delete-blob-overview).
...
- Для обеспечения высокого уровня устойчивости и доступности следует создавать виртуальные машины в группах доступности. При запланированных или незапланированных простоях группы доступности гарантируют, что по крайней мере одна виртуальная машина в наборе остается доступной.
   ...
   *Рис. 12. управляемые диски.*

**Подробнее:**

- Ознакомьтесь с [обзором управляемых дисков](/azure/virtual-machines/managed-disks-overview).
- Сведения о [преобразовании дисков в управляемые](/azure/virtual-machines/windows/convert-unmanaged-to-managed-disks).
- Узнайте, как [управлять доступностью виртуальных машин Windows в Azure](/azure/virtual-machines/manage-availability).
...
- Узнайте, как [перенаправлять данные Azure в средство SIEM](/azure/security-center/security-center-partner-integration).
 ...
 *Рис. 14. ведение журнала диагностики.*

**Подробнее:**

- Сведения о [сборе и использовании данных журнала](/azure/azure-monitor/essentials/platform-logs-overview).
- Узнайте, что поддерживается для [ведения журнала диагностики](/azure/azure-monitor/essentials/resource-logs-schema).
...
- В качестве простого примера можно создать оповещение, которое активируется при сканировании порта для NSG. Можно настроить сборник схем, который будет запускаться и блокировать IP-адрес источника сканирования.
...
- Сведения об [оповещениях](/azure/azure-monitor/alerts/alerts-overview).
- Узнайте о [безопасности модули PlayBook, которые отвечают на оповещения центра безопасности](/azure/security-center/workflow-automation).
...
- Благодаря этой общей модели ваша команда имеет возможность видеть среду Azure, которая помогает им быть профилактической при управлении системами в облаке.

  ![Снимок экрана панели мониторинга Azure.](./media/migrate-best-practices-security-management/dashboard.png)

  ![Снимок экрана панели мониторинга Azure. ](./media/migrate-best-practices-security-management/dashboard.png)
   *Рис. 16. панель мониторинга Azure.*
...
- Кроме того, можно зарегистрировать виртуальные машины Azure с помощью System Center Configuration Manager. Затем можно перенести рабочую нагрузку Configuration Manager в Azure и выполнять отчеты и обновления программного обеспечения с помощью единого веб-интерфейса.

  ![Схема обновлений виртуальных машин. ](./media/migrate-best-practices-security-management/updates.png)
   *Рис. 18. обновления виртуальных машин.*

**Подробнее:**

- Дополнительные сведения о [Управление обновлениями в Azure](/azure/automation/update-management/overview).
- Узнайте, как [интегрировать Configuration Manager с Управление обновлениями](/azure/automation/update-management/mecmintegration).
- [Ответы на вопросы](/mem/configmgr/core/understand/configuration-manager-on-azure) о Configuration Manager в Azure.
...
- На панели мониторинга "Отслеживание изменений" можно легко увидеть изменения, внесенные в серверную инфраструктуру.
  ...
  ![Снимок экрана с диаграммой управления изменениями.](./media/migrate-best-practices-security-management/change.png)

  _Рис. 19. Диаграмма управления изменениями._

**Подробнее:**

- Сведения об [отслеживании изменений](/azure/automation/change-tracking/overview).
