...
- Сведения о [антивредоносном по Майкрософт для Azure](https://docs.microsoft.com/azure/security/fundamentals/antimalware).
...
RBAC может назначать роли субъектам безопасности, таким как владелец, участник и читатель, и определения ролей (коллекция разрешений), которые определяют операции, которые могут быть выполнены ролями. RBAC также может задавать области, устанавливающие границы для той или иной роли. Область можно задать на нескольких уровнях, включая группу управления, подписку, группу ресурсов или ресурс.
...
- [Научитесь](https://docs.microsoft.com/azure/role-based-access-control/role-assignments-portal) управлять доступом с помощью RBAC и портал Azure.
...
На следующей схеме показан пример создания иерархии для управления с помощью групп Management.
...
В целях безопасности Azure Backup шифрует данные с помощью AES-256. Он отправляет его по протоколу HTTPS в Azure. Резервные копии неактивных данных в Azure шифруются с помощью [шифрования службы хранилища Azure](https://docs.microsoft.com/azure/storage/common/storage-service-encryption).
...
- Azure Monitor может помочь в области этих метрик и предоставлять ответы с предупреждениями, автомасштабированием, концентраторами событий и приложениями логики.
