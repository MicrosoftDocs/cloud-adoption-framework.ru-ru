...
4. Раздел **клиента** web.config файла для **смарсотел. Registration. Web** следует изменить, чтобы он указывал на новое расположение службы WCF. Это URL-адрес веб-приложения WCF, на котором размещена конечная точка службы.
