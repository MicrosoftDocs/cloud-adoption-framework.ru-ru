---
title: Повторное размещение в Azure и Управляемый экземпляр Базы данных SQL Azure
description: Узнайте, как повторно разместить локальное приложение на виртуальных машинах Azure с помощью Управляемый экземпляр Базы данных SQL Azure.
...
3. Веб-уровень будет перенесен с помощью миграции с помощью функции переноса и сдвига с использованием Site Recovery. Этот процесс подготовит локальную среду VMware, настроит и включит репликацию, а также осуществит миграцию виртуальных машин в Azure.

     ![Архитектура миграции](media/contoso-migration-rehost-vm-sql-managed-instance/migration-architecture.png)

### <a name="azure-services"></a>Службы Azure

Служба | Description | Стоимость
--- | --- | ---
...
После отработки отказа в Azure компания Contoso хочет подключиться к реплицированным виртуальным машинам в Azure. Для этого перед миграцией администратор Contoso должен выполнить несколько задач на локальной виртуальной машине:
...
14. По окончании регистрации на портале Azure они повторно проверяют, указаны ли серверы конфигурации и VMware на странице **Источник** в хранилище. Обнаружение может занять 15 минут или более.
...
3. При тестировании отработки отказа происходит следующее: выполняется проверка на соблюдение всех необходимых условий для миграции;
    b. обрабатываются данные для создания виртуальной машины Azure; если выбрана последняя точка восстановления, на основе этих данных создается точка восстановления;
    c. создается виртуальная машина Azure с использованием данных, обработанных на предыдущем шаге.
