...
description: Используйте платформу внедрения облаков для Azure, чтобы ознакомиться с рекомендациями по настройке сети для перенесенных рабочих нагрузок.
...
- Для каждой виртуальной сети следует назначить адресное пространство размером не больше диапазона CIDR /16. Виртуальных сетей позволяют использовать IP-адреса 65 536 и назначать меньший префикс, отличный от/16, например a/15, который имеет 131 072 адресов, приведет к тому, что лишние IP-адреса станут непригодными для использования в других местах. Важно не терять IP-адреса, даже если они находятся в частных диапазонах, определенных в RFC 1918.
...
- Ознакомьтесь с [часто задаваемыми вопросами о виртуальной сети Azure (часто ЗАдаваемые вопросы)](https://docs.microsoft.com/azure/virtual-network/virtual-networks-faq).
- Сведения об [ограничениях сети Azure](https://docs.microsoft.com/azure/azure-resource-manager/management/azure-subscription-service-limits?toc=/azure/virtual-network/toc.json#networking-limits).
...
. в[статье о](https://docs.microsoft.com/azure/virtual-wan/virtual-wan-about) виртуальной глобальной сети Azure.
...
Allow-Database-BusinessLogic | Разрешение трафика из группы безопасности приложений AsgLogic в группу безопасности приложений AsgDb. Приоритет этого правила выше, чем правило запрета базы данных-все, поэтому сначала обрабатывается это правило. Таким образом, трафик из группы безопасности приложений Асглогик разрешается, и весь остальной трафик блокируется. | Приоритет: 110<br/><br/> Источник: Асглогик<br/><br/> Исходный порт: *<br/><br/> Назначение: Асгдб<br/><br/> Порт назначения: 1433<br/><br/> Протокол: TCP.<br/><br/> Доступ: разрешить.
