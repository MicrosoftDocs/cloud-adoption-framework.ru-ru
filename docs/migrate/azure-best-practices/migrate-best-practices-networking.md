...
- Виртуальные сети могут содержать частные и общедоступные IP-адреса, определенные в [RFC 1918](https://tools.ietf.org/html/rfc1918), выраженные в нотации CIDR. Общедоступные IP-адреса, указанные в адресном пространстве виртуальной сети, недоступны из Интернета напрямую.
- Виртуальные сети могут подключаться друг к другу с помощью пиринга виртуальной сети. Подключенные виртуальные сети могут находиться в одном или в разных регионах. Поэтому ресурсы в одной виртуальной сети могут подключаться к ресурсам в других виртуальных сетях.
...
При планировании топологии виртуальной сети следует учитывать, как упорядочивать пространства IP-адресов, как реализовать сеть на основе концентраторов, как сегментирование виртуальных сетей в подсети, настройку DNS и реализацию Зоны доступности Azure.
...
- Перекрывающиеся адреса могут вызвать сети, которые не могут быть подключены, и маршрутизация, которая не работает должным образом. Если сети перекрываются, необходимо будет переконструировать сеть или использовать NAT.
...
## <a name="best-practice-implement-a-hub-and-spoke-network-topology"></a>Рекомендации. Реализация топологии сети "звезда"

Топология сети типа "звезда" изолирует рабочие нагрузки при совместном использовании служб, таких как идентификация и безопасность. Центр — это виртуальная сеть Azure, которая выступает в качестве центральной точки подключения.
Периферийные серверы — это виртуальные сети, которые подключаются к виртуальной сети концентратора с помощью пиринга. Общие службы развертываются в концентраторе, а отдельные рабочие нагрузки — в периферийных зонах.
...
- Конфигурация типа "звезда" обычно используется крупными компаниями. В меньших сетях можно организовать более простую и бюджетную топологию.
...
- Вы можете реализовать виртуальные сети в разных группах ресурсов и даже в разных подписках. Если вы настраиваете пиринг виртуальных сетей в разных подписках, эти подписки могут быть связаны с одним и тем же или разными клиентами Azure Active Directory (Azure AD). Это позволяет осуществлять децентрализованное управление каждой рабочей нагрузкой при совместном использовании служб, поддерживаемых в центральной сети.

![Схема топологии hub-and-лучевой ](./media/migrate-best-practices-networking/hub-spoke.png)
 _рис. 1. топология типа "звезда"._

**Подробнее:**

- Ознакомьтесь с [топологией звезды](/azure/architecture/reference-architectures/hybrid-networking/hub-spoke).
...
- Узнайте, как компания Contoso, вымышленная компания, [подготовила свою сетевую инфраструктуру к миграции](/azure/migrate/contoso-migration-infrastructure).
...
- В Azure Resource Manager можно указать DNS-серверы для виртуальной сети и сетевого интерфейса, но рекомендуется использовать параметр только в виртуальных сетях.
    ...
    _Рис. 2. DNS-серверы для виртуальной сети._

**Подробнее:**

- Сведения о [разрешении имен при использовании собственного DNS-сервера](/azure/migrate/contoso-migration-infrastructure).
...
- С Зоны доступности Azure предлагает соглашение об уровне обслуживания, которое составляет 99,99% времени работы виртуальной машины.

    ![Схема Зоны доступности в регионе Azure.](./media/migrate-best-practices-networking/availability-zone.png)

    _Рис. 3. Зоны доступности._
...
- Чтобы обеспечить отказоустойчивость зональные, можно развернуть стандартную подсистему балансировки нагрузки Azure с рабочими нагрузками с выходом в Интернет или уровнями приложений.

    ![Схема подсистемы балансировки нагрузки Azure уровня ](./media/migrate-best-practices-networking/load-balancer.png) _"Стандартный" рис. 4. Подсистема балансировки нагрузки._
...
- **VPN типа "сеть — сеть":** Вы устанавливаете подключение типа "сеть — сеть" между совместимым локальным VPN-устройством и VPN-шлюзом Azure, развернутым в виртуальной сети. Любой Полномочный локальный ресурс может получить доступ к виртуальным сетям. Обмен данными между сетями осуществляется по шифрованному туннелю через Интернет.
...
- VPN-шлюз также может передавать зашифрованный трафик между виртуальными сетями Azure по сети Microsoft.
...
- Протокол BGP является необязательным компонентом. Его можно использовать с Azure ExpressRoute и VPN-шлюзами на основе маршрутов для распространения локальных маршрутов BGP в виртуальные сети.

![Схема VPN-подключения типа "сеть — сеть". ](./media/migrate-best-practices-networking/vpn.png)
 _Рис. 5. VPN-подключение типа "сеть — сеть"._
...
- Узнайте о [планировании и проектировании VPN-шлюза](/azure/vpn-gateway/vpn-gateway-plan-design).
...
![Схема VPN с путем маршрута через неправильную цепь. ](./media/migrate-best-practices-networking/bgp1.png)
 _Рис. 6. сообщества BGP неоптимизированное подключение._

**Решение**.

Чтобы оптимизировать маршрутизацию для обоих офисов, необходимо знать, какой префикс относится к Azure `West US` и что из Azure `East US` . Эти сведения можно закодировать с помощью значений сообществ BGP.
...
![Схема VPN с путем маршрута в правильном канале. ](./media/migrate-best-practices-networking/bgp2.png)
 _Рис. 7. оптимизированное подключение сообществ BGP._
...
![Схема развертывания сети периметра виртуальной сети Azure. ](./media/migrate-best-practices-networking/perimeter.png)
 _Рис. 8. Развертывание сети периметра._
...
- Ознакомьтесь со сведениями о [группах безопасности сети (группы безопасности сети)](/azure/virtual-network/security-overview).
- Проверьте [теги службы, доступные для группы безопасности сети](/azure/virtual-network/security-overview#service-tags).
...
![Схема группы безопасности приложений ](./media/migrate-best-practices-networking/asg.png)
 _рис. 9. пример группы безопасности приложения._
...
- Сведения о [группах безопасности приложений](/azure/virtual-network/security-overview#application-security-groups).
...
![Схема конечных точек службы. ](./media/migrate-best-practices-networking/endpoint.png)
 _Рис. 10. конечные точки службы._
...
- Важно отметить, что для общедоступного IP-адреса этого уровня группа безопасности сети не настраивается автоматически. Необходимо настроить собственные и назначить правила для управления доступом. IP-адреса стандартного SKU имеют NSG и правила, назначенные по умолчанию.
...
- [Общедоступные IP-адреса в Azure](/azure/virtual-network/virtual-network-ip-addresses-overview-arm#public-ip-addresses)
...
В Azure есть функции безопасности на уровне платформы, включая брандмауэр Azure, брандмауэр веб-приложения и наблюдатель за сетями.
