...
В этой статье описывается, как вымышленная компания Contoso подготавливает свою локальную инфраструктуру к миграции, настраивает инфраструктуру Azure в ходе подготовки к миграции и ведет деловую деятельность в гибридной среде. Если вы используете этот пример для планирования собственных усилий по миграции инфраструктуры, учитывайте следующие соображения.
...
- Каждый филиал подключен локально к Интернету с помощью соединения бизнес-класс и применяет VPN-туннели IPSec для передачи данных в основной центр обработки данных. Такой подход позволяет постоянно подключать всю сеть и оптимизировать подключение к Интернету.
...
После настройки подписок Contoso может переходить к вопросу лицензирования продуктов корпорации Майкрософт. Стратегия лицензирования будет зависеть от ресурсов, которые Contoso хочет перенести в Azure, а также от выбора и развертывания виртуальных машин и служб в Azure.
...
В Exchange для использования зарезервированных экземпляров для конкретных экземпляров виртуальных машин необходимо хранить в течение длительного времени, Contoso может получить как скидку, так и приоритетную емкость. Использование [зарезервированных экземпляров Azure](https://azure.microsoft.com/pricing/reserved-vm-instances) вместе с преимущество гибридного использования Azure может сэкономить Contoso до 82% по обычной цене с оплатой по мере использования (по состоянию на апрель 2018).
...
- Дополнительные сведения об [Azure AD для Office 365](https://docs.microsoft.com/office365/enterprise/about-office-365-identity).
- Узнайте, как [Добавить подписку в существующий клиент Azure AD](https://docs.microsoft.com/azure/active-directory/fundamentals/active-directory-how-subscriptions-associated-directory).
...
2. В поле **создать каталог**укажите имя каталога, исходное доменное имя и регион, в котором должен быть создан каталог.
...
2. Чтобы использовать DNS-запись в Azure, необходимо зарегистрировать ее в регистраторе домена.
    ...
    - Для этого компании необходим доступ к DNS-серверу. Они входят в `contoso.com` домен и создают новую запись MX для записи DNS, предоставляемой Azure AD, с использованием указанных здесь сведений.
...
- Подписка Azure может иметь несколько групп ресурсов. Группа ресурсов существует в одной подписке.
- Кроме того, одна группа ресурсов может иметь несколько ресурсов. Ресурс принадлежит к одной группе ресурсов.
...
4. В окне **Подключение к AD DS**они указывают учетные данные для локального Active Directory (в форме `CONTOSO\admin` или `contoso.com\admin` ).
...
2. В поле **Добавить роль назначения роли**  >  **Role**> **участник**выбирает `ContosoCobRG` группу из списка. После этого группа появляется в списке **Выбранные элементы**.
3. Они повторяются с теми же разрешениями для других групп ресурсов (кроме `ContosoAzureAdmins` ), добавляя `Contributor` разрешения для учетной записи, соответствующей группе ресурсов.
4. Для `ContosoAzureAdmins` группы они назначают `Owner` роль.
...
При проектировании гибридной среды специалистам Contoso необходимо учитывать то, как стратегия обеспечения устойчивости и аварийного восстановления впишется в структуру региона. В общем, стратегии могут варьироваться от развертывания в одном регионе, которое использует функции платформы Azure, такие как домены сбоя и региональные пары для обеспечения устойчивости, до полной активной модели, в которой развернуты облачные службы и базы данных и обслуживают пользователей из двух регионов.

Компания Contoso выбрала срединный путь. Он развертывает приложения и ресурсы в основном регионе и сохраняет полную копию инфраструктуры в дополнительном регионе, чтобы она была готова к работе в качестве полной резервной копии в случае аварийного или регионального сбоя приложения.
...
Компания Contoso развернет зоны доступности, так как приложения нуждаются в масштабируемости, высокой доступности и устойчивости. Дополнительные сведения см. [в статье регионы и зоны доступности в Azure](https://docs.microsoft.com/azure/availability-zones/az-overview).
...
- Azure Backup шифрует транзитные данные с помощью AES-256. Резервные копии неактивных данных шифруются с помощью [Шифрование службы хранилища (SSE)](https://docs.microsoft.com/azure/storage/common/storage-service-encryption).

Contoso будет использовать Azure Backup с GRS на всех рабочих виртуальных машинах, чтобы обеспечить резервное копирование данных рабочей нагрузки и быстро восстановить их в случае сбоя. Дополнительные сведения см. в [обзоре Azure Backup](https://docs.microsoft.com/azure/backup/backup-overview).
...
Contoso развернет концентраторы в каждом регионе. Концентратор — это виртуальная сеть в Azure, которая выступает в качестве центральной точки подключения к локальной сети. Концентраторы виртуальных сетей будут подключены друг к другу с использованием глобального пиринга виртуальных сетей. Глобальный пиринг виртуальных сетей обеспечивает подключение между виртуальными сетями в разных регионах Azure.
...
При использовании модели звездообразного подключения, выбранной Contoso, необходимо продумать маршрутизацию трафика из локального центра обработки данных и из Интернета. Вот как компания Contoso решила управлять маршрутизацией как для `East US 2` концентраторов, так и для `Central US` :
...
- Компания Contoso будет реализовывать класс с частной сетью в Azure ( `10.0.0.0/8` ). Это работает, так как в настоящее время в локальной среде у него есть закрытое адресное пространство класса B ( `172.160.0.0/16` ), поэтому Contoso может быть уверенным в том, что диапазоны адресов не перекрываются.
...
| **Региональной** | **Разнообраз** | **Кэширующий узел** |
| --- | --- | --- |
...
Для контроллеров домена в сети VNET-PROD-EUS2 компании Contoso необходимо, чтобы трафик направлялся из концентратора или рабочей сети в регионе EUS2 в локальные среды через VPN-подключение. Для этого администраторы Contoso должны иметь следующие разрешения:

1. Специалисты компании должны **разрешить переадресацию трафика** и **разрешить конфигурации с транзитом через шлюз** для пирингового соединения. В нашем примере это будет подключение VNET-HUB-EUS2 to VNET-ПРОИЗВ-EUS2.
...
| **Регион** | **DC** | **Региональной** | **Подсеть** | **IP-адрес** |
| --- | --- | --- | --- | --- |
...
2. Они [создают группы доступности](https://docs.microsoft.com/azure/virtual-machines/windows/tutorial-availability-sets) в каждом расположении для виртуальной машины. Группы доступности используются для следующих задач:

    - разделения виртуальных машины на разные инфраструктуры в регионе Azure с помощью фабрики Azure;
    - обеспечить Contoso право на Соглашение об уровне обслуживания с доступностью в течение 99,95 % времени для виртуальных машин в Azure.
...
1. Они создают два новых сайта ( `AZURE-EUS2` и `AZURE-CUS` ) вместе с сайтом центра обработки данных ( `ContosoDatacenter` ).
...
Contoso [блокирует ресурсы](https://docs.microsoft.com/azure/azure-resource-manager/resource-group-lock-resources) следующим образом:
...
| `CatalogName` | Имя приложения или `ShareServices` для каталога услуг, который поддерживает ресурс. |
...
- **Центр безопасности Azure.** [Центр безопасности Azure](https://docs.microsoft.com/azure/security-center/security-center-intro) обеспечивает унифицированное управление безопасностью и расширенную защиту от угроз в гибридных облачных рабочих нагрузках. Используйте его для применения политик безопасности в рабочих нагрузках, для ограничения воздействия на угрозы и обнаружения атак и реагирования на них.
...
Компания Contoso будет использовать преимущества непрерывной оценки безопасности, которая наблюдает за безопасностью компьютеров, сетей, хранилищ, данных и приложений. для обнаружения потенциальных проблем безопасности.

- Центр безопасности анализирует состояние безопасности ресурсов вычислений, инфраструктуры и данных Contoso, а также приложений и служб Azure.
...
| Действие | Имя | Источник | Целевой объект | Порт |
| --- | --- | --- | --- | --- |
