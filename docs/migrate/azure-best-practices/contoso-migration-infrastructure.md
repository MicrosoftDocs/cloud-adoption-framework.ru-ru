...
- Получите общее представление об [управлении удостоверениями и доступом](https://www.microsoft.com/trustcenter/security/identity) в Azure. В частности, изучите [возможности Azure AD и расширение локальной службы Active Directory для работы в облаке](/azure/active-directory/identity-fundamentals). Вы можете загрузить полезную электронную книгу, посвященную [управлению удостоверениями и доступом в гибридной среде](https://azure.microsoft.com/resources/hybrid-cloud-identity).
- Azure предоставляет надежную сетевую инфраструктуру с различными вариантами гибридных подключений. Получите общее представление о [сетевых возможностях и управлении доступом к сети](/azure/security/security-network-overview).
- Ознакомьтесь с основами обеспечения [безопасности Azure](/azure/security/azure-security) и создания плана [управления](/azure/security/governance-in-azure).
...
- Для начала компания Contoso определила структуру (так называемый корпоративный каркас) для Соглашения о регистрации Enterprise. Чтобы понять принцип действия этого каркаса и спроектировать его, Contoso использовала [эту статью](/azure/azure-resource-manager/resource-manager-subscription-governance).
- На данном этапе в Contoso решили использовать для управления подписками функциональный подход.
  ...
  - Если в будущем компании Contoso потребуются дополнительные подписки, ей придется управлять доступом, политиками и соответствием требованиям. Для этого будут внедрены [группы управления Azure](/azure/azure-resource-manager/management-groups-overview), которые образуют дополнительный уровень над подписками.
...
Прогнозируемыми называются рабочие нагрузки, которые всегда должны быть доступны на работающих виртуальных машинах. Например, такие бизнес-приложения, как система SAP ERP. С другой стороны, непредсказуемые рабочие нагрузки — это переменные рабочие нагрузки, например виртуальные машины, включаемые в периоды высокой нагрузки и отключаемые, когда нагрузка небольшая.
...
- Ознакомьтесь со статьями [Идентификация в Office 365 и Azure Active Directory](https://support.office.com/article/understanding-office-365-identity-and-azure-active-directory-06a189e7-5ec6-4af2-94bf-a22ea225a7a9) и [Как связать или добавить подписку Azure в Azure Active Directory](/azure/active-directory/active-directory-how-subscriptions-associated-directory).
...
- администраторы могут использовать группы в Active Directory для реализации [управления доступом на основе ролей (RBAC)](/azure/role-based-access-control/role-assignments-portal) в Azure.

Для упрощения интеграции Contoso использует [средство Azure AD Connect](/azure/active-directory/connect/active-directory-aadconnect). Если установить и настроить это средство на контроллере домена, оно синхронизирует локальные удостоверения Active Directory с Azure AD.
...
1. На портале Azure администраторы Contoso выбирают **Azure Active Directory** > **Azure AD Connect** и загружают последнюю версию средства на сервер, используемый для синхронизации.
    ...
    ![Скачать Azure AD Connect](./media/contoso-migration-infrastructure/download-ad-connect.png)
...
- У Contoso есть прямое подключение к Azure. Если локальная служба Active Directory работает через прокси-сервер, прочитайте эту [статью](/azure/active-directory/connect/active-directory-aadconnect-troubleshoot-connectivity).
...
- Просмотрите дополнительные сведения о [регионах Azure](https://azure.microsoft.com/global-infrastructure/regions) и [связывании областей](/azure/best-practices-availability-paired-regions).
...
Компания Contoso реализует группы доступности, когда для рабочих нагрузок виртуальной машины требуется высокий уровень доступности. [Узнайте больше](/azure/virtual-machines/windows/manage-availability).
...
Contoso развернет зоны доступности, чтобы обеспечить масштабируемость, высокую доступность и устойчивость приложений. [Узнайте больше](/azure/availability-zones/az-overview).
...
- Azure Backup шифрует транзитные данные с помощью алгоритма AES 256. Неактивные данные резервных копий шифруются с помощью [Шифрования службы хранилища (SSE)](/azure/storage/common/storage-service-encryption?toc=%2fazure%2fstorage%2fqueues%2ftoc.json).

Компания Contoso будет использовать Azure Backup с GRS на всех рабочих виртуальных машинах, чтобы обеспечить резервное копирование данных рабочих нагрузок и их быстрое восстановление в случае сбоя или другого нарушения работы. [Узнайте больше](/azure/backup/backup-introduction-to-azure-backup).
...
Компания Contoso внедрит Azure Site Recovery для всех рабочих виртуальных машин, используемых для критически важных рабочих нагрузок. Это обеспечит минимальный простой при сбое в основном регионе. [Подробнее](/azure/site-recovery/site-recovery-overview)
...
В Contoso рассмотрели [несколько вариантов архитектуры](/azure/architecture/reference-architectures/hybrid-networking) для организации гибридных сетевых подключений между Azure и локальным центром обработки данных. [Узнайте больше](/azure/architecture/reference-architectures/hybrid-networking/considerations) о сравнении этих вариантов.
...
3. В ходе увеличения масштаба развертывания Azure компания Contoso установит подключение ExpressRoute между центром обработки данных и регионами Azure. В этом случае VPN-подключения типа "сеть-сеть" будут сохранены только для отработки отказа.
    - Дополнительные сведения о выборе между гибридным решением на базе VPN и ExpressRoute см. в [этой статье](/azure/architecture/reference-architectures/hybrid-networking/considerations).
...
Подробные сведения о пиринге сетей см. в этой [статье](/azure/virtual-network/virtual-network-peering-overview).
...
- У Contoso нет [решения IPAM](/windows-server/networking/technologies/ipam/ipam-top), поэтому компании нужно спланировать сетевую маршрутизацию без преобразования сетевых адресов (NAT).
...
2. Они создадут в каждом расположении группы доступности для виртуальной машины. Группы доступности используются для следующих задач:
    ...
    - обеспечить Contoso право на Соглашение об уровне обслуживания с доступностью в течение 99,95 % времени для виртуальных машин в Azure. [Узнайте больше](/azure/virtual-machines/windows/tutorial-availability-sets).
...
Azure предоставляет ряд средств управления, которыми можно воспользоваться в различных службах и на платформе Azure. Общие сведения о параметрах см. в этой [статье](/azure/security/governance-in-azure).
...
- **Политика** Политика Azure используются для применения правил к ресурсам и обеспечения их выполнения. Это гарантирует соответствие ресурсов корпоративным требованиям и соглашениям об уровне обслуживания.
...
Политики задают определение политики, а назначение политики определяет область ее применения. Область может варьироваться от группы управления до группы ресурсов. Дополнительные сведения о создании политик и управлении ими см. в этой [статье](/azure/governance/policy/tutorials/create-and-manage).
...
Подробные сведения о блокировках см. в этой [статье](/azure/azure-resource-manager/resource-group-lock-resources).
...
- **Центр безопасности Azure** Центр безопасности Azure обеспечивает унифицированное управление безопасностью и расширенную защиту от угроз для гибридных облачных рабочих нагрузок. С помощью центра безопасности можно применять политики безопасности для рабочих нагрузок, ограничить уязвимость перед угрозами, а также выявлять атаки и реагировать на них. [Узнайте больше](/azure/security-center/security-center-intro).
- **Группы безопасности сети (NSG)** Представляют собой фильтр (брандмауэр), содержащий перечень правил безопасности, которые (в случае их применения) разрешают или запрещают передачу сетевого трафика ресурсам, подключенным к виртуальным сетям Azure. [Узнайте больше](/azure/virtual-network/security-overview).
- **Шифрование данных** Шифрование дисков Azure — это возможность, которая помогает зашифровать диски виртуальных машин IaaS под управлением Windows и Linux. [Узнайте больше](/azure/security/azure-security-encryption-atrest).
...
![Отслеживание](./media/contoso-migration-infrastructure/monitoring.png)
