...
- Ознакомьтесь с основами обеспечения [безопасности Azure](https://docs.microsoft.com/azure/security/fundamentals/overview) и создания плана [управления](https://docs.microsoft.com/azure/governance).
...
В Exchange для использования зарезервированных экземпляров для конкретных экземпляров виртуальных машин необходимо хранить в течение длительного времени, Contoso может получить как скидку, так и приоритетную емкость. Используя [зарезервированные экземпляры Azure](https://azure.microsoft.com/pricing/reserved-vm-instances) вместе с преимущество гибридного использования Azure, компания Contoso может сэкономить до 82% по обычной цене с оплатой по мере использования (Апрель 2018).
...
- Узнайте больше об [Azure AD для Office 365](https://docs.microsoft.com/office365/enterprise/about-office-365-identity)и о том, как [Добавить подписку в существующий клиент Azure AD](https://docs.microsoft.com/azure/active-directory/active-directory-how-subscriptions-associated-directory).
...
2. В поле **создать каталог**укажите имя каталога, исходное доменное имя и регион, в котором должен быть создан каталог Azure AD.

    ![Создание Azure AD](./media/contoso-migration-infrastructure/azure-ad-create.png)

    > [!NOTE]
    > Созданный каталог имеет первоначальное доменное имя в формате **domain-name.onmicrosoft.com**. Это имя нельзя изменить или удалить. Вместо этого необходимо добавить свое зарегистрированное доменное имя в Azure AD.

### <a name="add-the-domain-name"></a>Добавление доменного имени

Чтобы использовать стандартное доменное имя, администраторам Contoso необходимо добавить его в качестве имени личного домена в Azure AD. Эта возможность позволяет им назначать привычные имена пользователей. Например, пользователь может войти с помощью адреса billg@contoso.comэлектронной почты, а не нужно. billg@contosomigration.microsoft.com
...
Администраторы Contoso теперь назначают роли группам Active Directory, которые они синхронизированы из локальной среды.

1. В группе ресурсов **ControlCobRG** они выбирают **Управление доступом (IAM)** > **Добавить назначение ролей**.
2. В **поле Добавление** > **роли**назначения роли > **участник**выбирается группа **контосокобрг** из списка. После этого группа появляется в списке **Выбранные элементы**.
...
Компания Contoso реализует группы доступности, когда для рабочих нагрузок виртуальной машины требуется высокий уровень доступности. [Подробнее.](https://docs.microsoft.com/azure/virtual-machines/windows/manage-availability)
...
Contoso развернет зоны доступности, чтобы обеспечить масштабируемость, высокую доступность и устойчивость приложений. [Подробнее.](https://docs.microsoft.com/azure/availability-zones/az-overview)
...
Компания Contoso внедрит Azure Site Recovery для всех рабочих виртуальных машин, используемых для критически важных рабочих нагрузок. Это обеспечит минимальный простой при сбое в основном регионе. [Подробнее](https://docs.microsoft.com/azure/site-recovery/site-recovery-overview)
...
- **Центр:** Виртуальная сеть концентратора в восточной части США 2 считается основным подключением к локальному центру обработки данных.
- **Виртуальных сетей:** Периферийную виртуальных сетей в восточной части США 2 можно использовать для изоляции рабочих нагрузок, если это необходимо. Помимо виртуальной сети-концентратора у Contoso в восточной части США 2 есть две периферийные виртуальные сети.
  - **Vnet-dev-EUS2**. Эта виртуальная сеть будет предоставлять отделу разработки и тестирования полностью функциональную сеть для проектов разработки. Она будет выступать в качестве рабочей пилотной площадки и использовать инфраструктуру рабочей среды.
    - **Vnet-EUS2**. В этой сети будут размещены рабочие компоненты Azure IaaS.
...
**Виртуальная сеть** | **Разнообраз** | **Кэширующий узел**
--- | --- | ---
...
- **Центр:** Виртуальная сеть концентратора в центральной части США считается вторичной точкой подключения к локальному центру обработки данных, а периферийная виртуальных сетей в центральной части США может использоваться для изоляции рабочих нагрузок, управляемых отдельно от других периферийных зон.
...
- **Подсети:** Подсети будут спроектированы так же, как и в восточной части США 2.
...
2. Они создадут в каждом расположении группы доступности для виртуальной машины. Группы доступности используются для следующих задач:
    ...
    - обеспечить Contoso право на Соглашение об уровне обслуживания с доступностью в течение 99,95 % времени для виртуальных машин в Azure. [Подробнее.](https://docs.microsoft.com/azure/virtual-machines/windows/tutorial-availability-sets)
...
- **Центр безопасности Azure:** Центр безопасности Azure обеспечивает унифицированное управление безопасностью и расширенную защиту от угроз в гибридных облачных рабочих нагрузках. С помощью центра безопасности можно применять политики безопасности для рабочих нагрузок, ограничить уязвимость перед угрозами, а также выявлять атаки и реагировать на них. [Подробнее.](https://docs.microsoft.com/azure/security-center/security-center-intro)
- **Группы безопасности сети (группы безопасности сети):** NSG — это фильтр (брандмауэр), который содержит список правил безопасности, которые при применении позволяют разрешать или запрещать сетевой трафик к ресурсам, подключенным к Azure виртуальных сетей. [Подробнее.](https://docs.microsoft.com/azure/virtual-network/security-overview)
- **Шифрование данных:** Шифрование дисков Azure — это функция, которая помогает шифровать диски виртуальных машин IaaS под управлением Windows и Linux. [Подробнее.](https://docs.microsoft.com/azure/security/fundamentals/encryption-atrest)
...
**Действие** | **Имя** | **Источник** | **Target** | **порт**.
--- | --- | --- | --- | ---
