...
- **Масштабирование.** По мере успешного развития бизнеса ИТ-отдел компании Contoso должен предоставить системы, которые могут расти в одном темпе.
...
[Служба приложений Azure](https://azure.microsoft.com/services/app-service/containers) | Оперативно создавайте, развертывайте и масштабируйте веб-приложения, мобильные приложения и приложения API на любой платформе. | Плата за план службы приложений начисляется посекундно. [Дополнительные сведения](https://azure.microsoft.com/pricing/details/app-service/windows)
...
1. Они открывают командную строку разработчика и используют командную `az login` для подписки Azure.
...
3. Для приложения-функции они создают другой проект Azure DevOps (SmartHotelPetChecker) и импортируют в него репозиторий Git [PetChecker](https://github.com/sonahander/SmartHotel360-PetCheckerFunction).
...
8. В окне **триггеры**выберите, чтобы **включить непрерывную интеграцию**, а затем выберите **Сохранить & очередь** , чтобы начать сборку.
