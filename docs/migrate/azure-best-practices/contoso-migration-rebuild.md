...
6. Они выполняют проверку подлинности `az login` Azure CLI, выполняя команду и следуя инструкциям по проверке подлинности с помощью веб-браузера. Дополнительные сведения о входе с помощью Azure CLI см. в [этой статье](https://docs.microsoft.com/cli/azure/authenticate-azure-cli?view=azure-cli-latest).
...
9. После завершения развертывания устанавливается программа `kubectl` командной строки. Средство уже установлено на Azure Cloud Shell.
...
Contoso создает проект Azure DevOps, настраивает сборку непрерывной интеграции, чтобы создать контейнер, а затем отправляет его в ACR. В инструкциях этого раздела используется репозиторий [SmartHotel360-серверной части](https://github.com/Microsoft/SmartHotel360-Backend) .
...
15. Они насматривают приложение для проверки правильности работы приложения "Проверка Pet" в [http://smarthotel360public.azurewebsites.net/Pets](http://smarthotel360public.azurewebsites.net/Pets).
...
- Contoso может использовать [георепликацию для SKU контроля доступа уровня "Премиум"](https://docs.microsoft.com/azure/container-registry/container-registry-geo-replication).
