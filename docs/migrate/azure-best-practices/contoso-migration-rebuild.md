...
6. Они проверяют подлинность Azure CLI, выполняя `az login` команду и следуя инструкциям по проверке подлинности с помощью веб-браузера. Дополнительные сведения о [входе в систему с помощью Azure CLI](/cli/azure/authenticate-azure-cli?view=azure-cli-latest).
