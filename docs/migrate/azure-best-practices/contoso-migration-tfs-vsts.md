...
6. **Завершите рабочую миграцию.** После завершения миграции с помощью пробного выполнения администраторы Contoso выполняют окончательную миграцию, обновляя **import.jsв** файле и повторно выполняя импорт.
...
- SqlPackage.exe в SQL Server Data Tools используется для создания DACPAC. Существует несколько версий SqlPackage.exe с SQL Server Data Tools, которые находятся в папках с такими именами, как 120, 130 и 140. Для подготовки DACPAC важно использовать правильную версию.
