...
| **Управление** | Группы управления ресурсами должны создаваться для различных отделов вместе с группами ресурсов для управления всеми переносимыми базами данных SQL. Все ресурсы должны быть помечены сведениями о подразделении для требований к возврату. |
...
Оценка определяет, что компания Contoso использует в основном. Приложения на основе NET. Однако в некоторых проектах используются другие технологии, такие как PHP и Node.js. Приобретенные в систему поставщики системы также предоставили приложения, не основанные на .NET. Компания Contoso определила следующие приложения:
...
> [!NOTE]
> Компания Contoso обнаружила различные базы данных с открытым кодом на этапе оценки. По отдельности, они применяют инструкции по [переносу баз данных с открытым исходным кодом в Azure](./contoso-migration-oss-db-to-azure.md) для планирования миграции.
...
| Управляемый экземпляр SQL Azure | SQL Server (дополнительные функции) | Эти базы данных используют триггеры и другие [сложные концепции](/azure/azure-sql/managed-instance/transact-sql-tsql-differences-sql-server#service-broker) , такие как пользовательские типы .NET, брокеры служб и т. д. | [Помощник по миграции данных](/sql/dma/dma-overview), [репликация транзакций](/azure/azure-sql/managed-instance/replication-transactional-overview) | [BACPAC](/sql/relational-databases/data-tier-applications/import-a-bacpac-file-to-create-a-new-user-database), [bcp](/sql/tools/bcp-utility?view=sql-server-ver15), [собственная Архивация и восстановление](/azure/azure-sql/managed-instance/restore-sample-database-quickstart) | 2 Тиб-8 тиб | [Ссылка](/azure/dms/tutorial-sql-server-managed-instance-online) |
| SQL Server на виртуальных машинах Azure (IaaS) | SQL Server (интеграция со сторонними разработчиками) | SQL Server должны иметь [неподдерживаемые функции SQL управляемый экземпляр](/azure/azure-sql/managed-instance/transact-sql-tsql-differences-sql-server#service-broker) (брокеры служб с несколькими экземплярами, поставщики служб шифрования, буферный пул, уровни совместимости ниже 100, зеркальное отображение базы данных, FILESTREAM, polybase, все, что требует доступа к файловым ресурсам, внешним сценариям, расширенным хранимым процедурам и другим) или программному обеспечению сторонних разработчиков для поддержки действий базы данных. | [Репликация транзакций](/azure/azure-sql/managed-instance/replication-transactional-overview) | [BACPAC](/sql/relational-databases/data-tier-applications/import-a-bacpac-file-to-create-a-new-user-database), [bcp](/sql/tools/bcp-utility?view=sql-server-ver15), [репликация моментальных снимков](/azure/azure-sql/managed-instance/replication-transactional-overview), [собственная Архивация и восстановление](/azure/azure-sql/managed-instance/restore-sample-database-quickstart), преобразование физического компьютера в виртуальную машину | 4 гиб-64 тиб | [Ссылка](/azure/azure-sql/virtual-machines/windows/migrate-to-vm-from-sql-server) |
...
- Компания Contoso должна обеспечить безопасность своих новых рабочих нагрузок базы данных Azure. [Подробнее.](/azure/azure-sql/database/security-overview)
...
- Включите [защитник Майкрософт для идентификации](/azure/azure-sql/database/threat-detection-overview) для базы данных SQL Azure.
...
- [Эластичные пулы](/azure/azure-sql/database/service-tiers-dtu) должны быть реализованы для баз данных с совместимыми шаблонами использования ресурсов.
...
Проект Azure DevOps был разработан для изучения процесса миграции SQL и соответствует облачной инфраструктуре внедрения. Этот проект поможет вам выполнить необходимые ключевые решения. [Выберите эту ссылку](https://azuredevopsdemogenerator.azurewebsites.net/?name=sqlmigration) для перехода к проекту Azure DevOps.
