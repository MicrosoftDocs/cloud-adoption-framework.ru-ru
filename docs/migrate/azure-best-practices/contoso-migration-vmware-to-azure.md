...
Частное облако решения Azure VMware подключается к виртуальной сети Contoso в Azure с помощью другого подключения Azure ExpressRoute. Global Reach ExpressRoute будет разрешена для [прямого подключения](/azure/azure-vmware/concepts-networking#on-premises-interconnectivity) локальных виртуальных машин к виртуальным машинам, работающим в частном облаке решения VMware Azure. Для включения Global Reach требуется номер SKU "Премиум" Premium.
...
Для управления частными облаками решения Azure VMware используется портал решения Azure VMware. Contoso имеет собственную vCenter Server в своем собственном домене управления.
...
1. Группа создает VPN-шлюз ExpressRoute, выберет правильный номер SKU, а затем нажмите кнопку Обзор и **Создание**.
    ...
    ![Снимок экрана: панель "Создание шлюза виртуальной сети" * *.](./media/contoso-migration-vmware-to-azure/create-virtual-network-gateway.png)
...
Дополнительные сведения см. в [статье Установка хккс для решения VMware для Azure](/azure/azure-vmware/tutorial-deploy-vmware-hcx).
...
- Группа Contoso развертывает ХККС с помощью пакета OVF, предоставляемого VMware.
   ...
   ![Снимок экрана: окно "* * развертывание шаблона OVF * *".](./media/contoso-migration-vmware-to-azure/configure-template.png)

   Чтобы установить и настроить ХККС для частного облака решения Azure VMware, см. статью [Установка хккс для Azure VMware](/azure/azure-vmware/tutorial-deploy-vmware-hcx).
...
- По мере того, как команда настраивает ХККС, вы решили включить миграцию и другие параметры, включая аварийное восстановление.
   ...
   ![Снимок экрана: окно "Создание сетки службы * *" для настройки ХККС.](./media/contoso-migration-vmware-to-azure/hcx-manager.png)
