...
После отработки отказа в Azure компания Contoso хочет подключиться к виртуальным машинам Azure. Чтобы сделать это, администраторы Contoso должны выполнить следующие действия:
...
Чтобы продолжить, они подтверждают завершение, выбрав **Да, я сделал это**. Компания Contoso выполняет миграцию только одной виртуальной машины в этом сценарии, которая не требует планирования развертывания.
...
6. В **параметрах репликации** > **настроить параметры репликации**, убедитесь, что применена правильная политика репликации, а затем выберите **включить репликацию**. Служба Mobility Service установится автоматически.
