...
- В настоящее время приложение распределено между двумя виртуальными`OSTICKETWEB` машинами (и `OSTICKETMYSQL`).
...
- У них уже есть [инфраструктура Azure](./contoso-migration-infrastructure.md) , поэтому Contoso необходимо только добавить и настроить репликацию виртуальных машин с помощью средства миграции сервера Azure Migration Server.
...
--- | --- | ---
[Миграция Azure](https://docs.microsoft.com/azure/migrate/migrate-overview) | Компания Contoso использует службу "Миграция Azure" для оценки виртуальных машин VMware. Служба "Миграция Azure" оценивает миграционную пригодность компьютеров. Она обеспечивает оценку размера и стоимости для работы в Azure. | Служба " [Миграция Azure](https://azure.microsoft.com/pricing/details/azure-migrate) " доступна без дополнительной платы. Однако оплата может взиматься в зависимости от средств (первой стороны или ISV), которые вы решили использовать для оценки и миграции.
...
> - **Шаг 2. Подготовка локальной среды VMware для миграции сервера миграция в Azure.** Они подготавливают учетные записи для обнаружения ВМ и подготавливают подключение к виртуальной машине Azure после переноса.
...
> - **Шаг 4. Миграция виртуальной машины приложения с миграцией Azure миграция сервера.** Они выполняют тестовую миграцию, чтобы убедиться, что все работает, а затем выполните полную миграцию, чтобы переместить виртуальную машину в Azure.
...
- Подготовленное и настроенное средство миграции сервера Azure (OVA).
...
2. Подготавливает средство миграции сервера в Azure Migration.
...
## <a name="step-2-prepare-on-premises-vmware-for-azure-migrate-server-migration"></a>Шаг 2. Подготовка локальной среды VMware для миграции сервера миграция Azure
...
- Чтобы получить доступ к виртуальной машине Azure, перед миграцией включите SSH на локальной виртуальной машине Linux. Для Ubuntu эту процедуру можно выполнить с помощью следующей команды: **Sudo apt-get ssh install -y**.
...
## <a name="step-4-migrate-the-vm-with-azure-migrate-server-migration"></a>Шаг 4. Перенос виртуальной машины с миграцией Azure миграция сервера
...
6. После добавления подсети создается правило виртуальной сети, которое разрешает доступ из подсети базы данных в рабочей сети.
...
3. Они перезапускают службу `systemctl restart apache2`с помощью.
