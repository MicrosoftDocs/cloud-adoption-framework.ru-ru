...
При планировании и проектировании для миграции Azure основное внимание уделяется затратам на обеспечение долгосрочного успеха. Во время проекта миграции очень важно, чтобы все группы (например, группы "Финансы", "Управление" и "Разработка приложений") понимали затраты, связанные с этим процессом.
...
- После миграции необходимо оптимизировать затраты, постоянно отслеживать рабочие нагрузки и планировать будущие структуры потребления. Перенесенные ресурсы могут начинаться с одного типа рабочей нагрузки, но из-за их использования, затрат и перенаправления требований бизнеса на другой тип с течением времени.
...
- Используйте Azure Reserved VM Instances.
...
Вы можете использовать несколько средств для прогнозирования ежемесячного счета за перенесенные рабочие нагрузки.
...
- Служба **"миграция Azure":** Чтобы оценить затраты, необходимо проверить и учитывать все ресурсы, необходимые для выполнения рабочих нагрузок в Azure. Чтобы получить эти данные, необходимо провести инвентаризацию ресурсов, включая серверы, виртуальные машины, базы данных и хранилище. Для сбора этой информации можно использовать службу "Миграция Azure".
  ...
  - Оценка службы "Миграция Azure" содержит предполагаемые расходы.
     ...
     *" рис. 2. Оценка службы "миграция Azure".*
...
| Тип данных | Подробнее | Использование |
| --- | --- | --- |
...
| **Файлы** | Управляемые общие файловые ресурсы, доступ к которым осуществляется через SMB 3,0. | Используется при переносе локальных файловых ресурсов и для предоставления нескольких прав доступа или подключений к данным файлов. |
...
| Уровень доступа | Подробнее | Использование |
| --- | --- | --- |
...
| **Здорово** | Меньше затрат на хранение, чем на горячий. Более высокие расходы на доступ, чем горячий. <br><br> Хранение не менее чем 30 дней. | Хранение краткосрочных. Данные доступны, но доступны редко. |
| **Архив** | Используется для отдельных блочных BLOB-объектов. <br><br> Наиболее экономный способ хранения. Доступ к данным дороже, чем для горячего и холодного уровней хранилищ. | Используйте для данных, которые могут допускать несколько часов задержки при извлечении и останутся на уровне не менее 180 дней. |
...
| Тип учетной записи | Подробнее | Использование |
| --- | --- | --- |
...
| **Общий назначение v1** | Распределение по уровням доступа не поддерживается. Не поддерживает ZRS. | Используйте, если приложениям требуется классическая модель развертывания Azure. |
...
- Сведения об [импорте и экспорте Azure](/azure/import-export/storage-import-export-service).
 ...
 *Рис. 3. Azure reserved VM Instances.*
...
- Ознакомьтесь [с руководством по ценам SQL Server на виртуальных машинах Azure](/azure/azure-sql/virtual-machines/windows/pricing-guidance).
...
- Узнайте, как [подключиться к Azure consumption Insights в Power BI Desktop](/power-bi/connect-data/desktop-connect-azure-consumption-insights).
...
Корпорация Майкрософт предоставляет службы управления затратами Azure и выставления счетов для помощи в отслеживании расходов. Эта служба отвечает за следующие задачи:
...
С помощью службы "Управление затратами Azure" и "выставление счетов" можно:
...
- Создайте бюджет для финансовой отчетности.
  ...
  - Вы можете экспортировать данные управления затратами в службу хранилища Azure, чтобы проанализировать их.

  ![Снимок экрана с бюджетом управления затратами Azure. ](./media/migrate-best-practices-costs/budget.png)
   *Рис. 4. Управление затратами Azure + бюджет на оплату.*

- Выполните анализ затрат для анализа и анализа затрат на организацию, чтобы понять, как начисляются затраты, и выявление тенденций расходов.
   ...
   *рис. 5. Управление затратами Azure + анализ выставления счетов.*

- Получите рекомендации помощника по Azure, которые покажут, как можно оптимизировать и повысить эффективность.

**Подробнее:**

- Прочитайте [Обзор службы управления затратами Azure и выставления счетов](/azure/cost-management-billing/cost-management-billing-overview).
- Узнайте, как [оптимизировать инвестиции в облако с помощью службы управления затратами Azure и выставления счетов](/azure/cost-management-billing/costs/cost-mgt-best-practices).
- Сведения об [управлении затратами Azure и отчетах о выставлении счетов](/azure/cost-management-billing/cloudyn/use-reports).
...
Постоянно оценивайте рабочие нагрузки виртуальной машины, чтобы определить базовые показатели. Например, если Рабочая нагрузка интенсивно используется с понедельника по пятницу, с 8 до 6 РМ, но вряд ли использовались за пределами этих часов, можно понизить нагрузку на виртуальные машины за пределами пикового времени. Это может приводить к изменению размеров виртуальной машины или потребовать использования масштабируемого набора виртуальных машин для автоматического масштабирования виртуальных машин. Некоторые компании отменяют виртуальные машины с помощью календаря, который указывает, когда они должны быть доступны и когда они не нужны.

Вы можете отслеживать использование виртуальной машины с помощью средств Майкрософт, таких как управление затратами Azure, выставление счетов, Azure Monitor и помощник по Azure. Доступны также сторонние средства.
...
- Рекомендуется оценить и настроить ведение журнала немедленно после завершения миграции, чтобы не тратить деньги на сохранность ненужных журналов.
...
- Воспользуйтесь преимуществами уровней доступа для хранилища общего назначения версии 2, перемещая менее важные данные с горячего уровня на интересные или архивные уровни.
...
- Получите ценные сведения об использовании с [API-интерфейсы выставления счетов Azure](/azure/cost-management-billing/manage/consumption-api-overview#usage-details-api).

## <a name="best-practice-implement-serverless-technologies"></a>Рекомендации. Реализация бессерверных технологий

Рабочие нагрузки виртуальных машин часто переносятся "как есть", чтобы избежать простоев. Часто виртуальные машины могут размещать задачи, которые являются временными, запускаться в течение короткого периода времени или занимать много часов. К примерам относятся виртуальные машины, на которых выполняются запланированные задачи, такие как планировщик заданий Windows или сценарии PowerShell. Если эти задачи не выполняются, затраты на хранение виртуальных машин и дисков не изменяются.

После переноса и тщательного рассмотрения этих типов задач можно рассмотреть возможность переноса их на бессерверные технологии, такие как функции Azure или пакетные задания Azure. Эти решения могут сократить расходы, и вам больше не нужно управлять виртуальными машинами и обслуживать их.
...
- Ознакомьтесь с рекомендациями [по обеспечению безопасности и управлению](./migrate-best-practices-security-management.md) после переноса рабочих нагрузок в Azure.
- Ознакомьтесь с рекомендациями по работе с [сетью](./migrate-best-practices-networking.md) после переноса рабочих нагрузок в Azure.
