---
title: Рефакторинг приложения Linux в службе приложений Azure и базе данных для MySQL
...
Приложение службы поддержки osTicket, используемое в этом примере, предоставляется в виде открытого кода. Если вы хотите использовать его для тестирования, вы можете скачать его из [репозитория остиккет на сайте GitHub](https://github.com/osTicket/osTicket).
...
- **Повышение устойчивости.** В прошлом проблемы возникают только с системными, затронутыми внутренними пользователями. В новой бизнес-модели эти проблемы будут влиять на внешних пользователей, и компании Contoso потребуется, чтобы приложение работало постоянно.
...
- База данных приложений будет перенесена в службу "база данных Azure для MySQL" с помощью Azure Database Migration Service (DMS). Будет создана локальная резервная копия локальной базы данных. Затем будет выполнено ее восстановление в Базу данных Azure для MySQL.
...
2. После подготовки инфраструктуры Azure она переносит базу данных с помощью Azure Database Migration Service (DMS).
...
[База данных Azure для MySQL](https://docs.microsoft.com/azure/mysql) | База данных расположена на ядре сервера MySQL с открытым исходным кодом. Она предоставляет полностью управляемую корпоративную базу данных MySQL для сообщества для разработки и развертывания приложений. | Ценообразование основано на вычислении, хранении и резервном копировании. [Подробнее.](https://azure.microsoft.com/pricing/details/mysql)
...
> - **Шаг 4. Перенос базы данных.** Они переносят базу данных с помощью Azure Database Migration Service (DMS).
...
1. Они создают ресурс веб-приложения в основном регионе "Восточная часть США 2" (**остиккет-eus2**) из Azure Marketplace.
...
2. Теперь специалисты компании настраивают диспетчер трафика с конечными точками. Они компании добавляют веб-приложение в регионе "Восточная часть США 2" как основной сайт (**osticket eus2**), а приложение в регионе "Центральная часть США" — как дополнительный (**osticket cus**).
...
Существует несколько способов перемещения базы данных MySQL. Каждый вариант требует создания экземпляра базы данных Azure для MySQL для целевого объекта. После создания можно выполнить миграцию, используя два пути:

- 4A: Azure Database Migration Service
- 4b: резервное копирование и восстановление MySQL Workbench

### <a name="step-4a-migrate-the-database-azure-database-migration-service"></a>Шаг 4A. Перенос базы данных (Azure Database Migration Service)
...
- Убедитесь, что выполнены все необходимые условия для миграции.
  - Источник MySQL Server должен соответствовать версии, поддерживаемой базой данных Azure для MySQL. База данных Azure для MySQL поддерживает версию-MySQL Community Edition, InnoDB подсистему и миграцию между исходными и целевыми версиями.
  - Включите ведение двоичного журнала в файле my. ini (Windows) или My. cnf (UNIX). Несоблюдение этого действия приведет к сбою `Error in binary logging. Variable binlog_row_image has value 'minimal'. Please change it to 'full'. For more information, see https://go.microsoft.com/fwlink/?linkid=873009` мастера миграции.
...
4. В редакторе они обновляют сведения о базе данных, в частности для **дбхост** и **дбусер**.
...
1. В основном веб-приложении (**остиккет-eus2**) они открывают **вариант развертывания** и устанавливают источник в **GitHub**.
