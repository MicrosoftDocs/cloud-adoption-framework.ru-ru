...
- **Ограничение риска.** Приложение SmartHotel360 критически важно для бизнеса компании Contoso. Ей нужно без рисков перенести приложение в Azure.
...
- После завершения миграции локальные виртуальные машины в центре обработки данных Contoso будут списаны.
...
Компания Contoso перенесет клиентские виртуальные машины и базы данных приложения на виртуальные машины Azure с помощью средства миграции сервера миграция Azure без агента.
...
> - **Шаг 2. Подготовка локальной среды VMware для миграции сервера миграция в Azure.** Они подготавливают учетные записи для обнаружения ВМ и подготавливают подключение к виртуальным машинам Azure после миграции.
...
> - **Шаг 4. Миграция виртуальных машин с миграцией Azure миграция сервера.** Они выполняют тестовую миграцию, чтобы убедиться, что все работает, а затем выполните полную миграцию, чтобы переместить виртуальные машины в Azure.
...
1. В списке **цели** > **серверы** > **Миграция Azure: миграция сервера**выберите **тестировать перенесенные серверы**.
...
- Безопасное хранение данных. Специалисты Contoso выполняют резервное копирование данных, содержащихся в виртуальных машинах, с помощью службы Azure Backup. [Подробнее.](https://docs.microsoft.com/azure/backup/backup-introduction-to-azure-backup?toc=/azure/virtual-machines/linux/toc.json)
