...
- Компания Contoso решила не переходить на базу данных Azure для MySQL, так как в будущем рассматривается использование модели MariaDB ColumnStore и базы данных Graph.
...
MySQL использует `x.y.z` схему управления версиями, где `x` — основная версия, `y` является дополнительной версией, а `z` — версией исправления.

Сейчас Azure поддерживает версии MySQL 10.2.25 и 10.3.16.
...
- Создайте новый проект Database Migration Service:
   ...
   *рис. 3. Проект Azure Database Migration Service.*
...
Убедитесь, что резервное копирование экземпляров базы данных Azure для MySQL выполняется с помощью геовосстановления, чтобы резервные копии можно было использовать в парном регионе, если происходит региональный сбой.
