...
1. На портале Azure последовательно выберите элементы **+ Создать ресурс** > **Средства управления** > **Backup и Site Recovery**.
...
1. В меню **Ресурс** последовательно выберите элементы **Site Recovery** > **Подготовка инфраструктуры** > **Цель защиты**.
1. На вкладке **Цель защиты** выберите объекты для миграции.
    1. **VMware.** Последовательно выберите элементы **В Azure** > **Да, с гипервизором VMware vSphere**.
    1. **Физический компьютер.** Последовательно выберите элементы **В Azure** > **Без виртуализации или иное**.
    1. **Hyper-V**. Последовательно выберите элементы **В Azure** > **Да, с Hyper-V**. Если управление виртуальными машинами Hyper-V осуществляется с помощью VMM, выберите **Да**.
...
1. Настройте целевую среду.
    1. Последовательно выберите элементы **Подготовка инфраструктуры** > **Цель** и выберите нужную подписку Azure.
...
1. В разделе **Параметры** > **Реплицированные элементы** выберите компьютер и щелкните элемент **Отработка отказа**.
...
1. Выберите **Завершение работы виртуальной машины перед началом отработки отказа**. Служба Site Recovery попытается завершить работу виртуальных машин, прежде чем активировать отработку отказа. Отработка отказа продолжится, даже если их не удастся отключить. На странице "Задания" можно следить за ходом отработки отказа.
1. Убедитесь, что виртуальная машина Azure отображается в Azure должным образом.
1. В разделе **Реплицированные элементы** выберите и удерживайте (или щелкните правой кнопкой мыши) виртуальную машину и выберите элемент **Завершить миграцию**.
...
Помощник по миграции данных поможет вам перейти на современную платформу данных благодаря обнаружению проблем с совместимостью, которые могут влиять на функции базы данных в новой версии SQL Server или Базы данных SQL Azure. DMA предлагает рекомендации по повышению производительности и надежности целевой среды, а также позволяет переместить схему, данные и неавтономные объекты с исходного сервера на целевой сервер.
