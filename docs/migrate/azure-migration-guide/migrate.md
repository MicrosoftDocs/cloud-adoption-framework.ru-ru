...
description: Запустите миграцию в Azure, определив соответствующие инструменты, включая собственные средства, сторонние средства и средства управления проектами.
author: matticusau
ms.author: mlavery
ms.date: 08/08/2019
ms.topic: guide
...
1. Щелкните **Выбор средства оценки** > **Azure Migrate: Оценка сервера** >  **Далее**.
1. Выберите **Review + add tools** (Проверка и добавление инструментов) и проверьте конфигурацию. Выберите **Добавить средства** , чтобы запустить задание, чтобы создать проект миграции и зарегистрировать выбранные решения.
...
1. На вкладке **Цель защиты** выберите объекты для миграции.
    1. **VMware:** Выберите **Azure > да с помощью низкоуровневой оболочки VMware vSphere**.
    1. **Физический компьютер:** Выберите **Azure >, которые не виртуализированы или другие**.
    1. **Hyper-V:** Выберите **Azure > да с помощью Hyper-V**. Если управление виртуальными машинами Hyper-V осуществляется с помощью VMM, выберите **Да**.
...
1. Настройте целевую среду.
    1. Выберите **подготовить инфраструктуру > целевой объект**, а затем выберите подписку Azure, которую хотите использовать.
...
1. Завершите работу мастера **создания службы миграции** , а затем выберите **создать**.
...
1. Скачайте и установите Помощник по миграции данных из [центра загрузки Майкрософт](https://www.microsoft.com/download/details.aspx?id=53595).
1. Создайте оценку, выбрав новый значок **(+)** , а затем выберите тип проекта **оценки** .
1. Задайте тип исходного и целевого сервера, а затем нажмите кнопку **создать**.
...
1. Нажмите кнопку **Далее** , чтобы начать оценку.
...
- **Запись:** Первый этап SQL Server A/B-тестирования — запись трассировки на исходном сервере. Исходным сервером обычно является рабочий сервер.
- **Воспроизведение:** Вторым этапом SQL Server A/B является воспроизведение файла трассировки, который был захвачен на целевых серверах. Затем выполняется сбор расширенных трассировок воспроизведения для анализа.
- **Анализ:** Последним шагом является создание аналитического отчета с помощью трассировок воспроизведения. Аналитический отчет поможет получить полезные сведения о влиянии предлагаемого изменения на производительность.
...
Унификлауд — это служба ISV, которая предоставляет средства для оценки, миграции и модернизации автоматизации.
...
- [Microsoft Planner](https://tasks.office.com)— простой, визуальный способ организации своей работы.
- [Microsoft Project](https://products.office.com/project/project-and-portfolio-management-software): Управление проектами и портфелем, Управление емкостью ресурсов, управление финансами, расписание и управление расписанием.
- [Microsoft](https://products.office.com/microsoft-teams)teams: средство для совместной работы и обмена данными. Teams также интегрируется с Планировщиком и другими инструментами для улучшения совместной работы.
- [Azure DevOps](https://dev.azure.com). шаблон плана для инфраструктуры внедрения облачных технологий не требуется для использования Azure DevOps. Вы можете использовать службу без шаблона и управлять инфраструктурой как кодом или использовать рабочие элементы и доски для управления проектами. По мере развития ваша организация может воспользоваться возможностями CI/CD.
